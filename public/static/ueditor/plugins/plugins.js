...
UE.plugins['formtext'] = function () {
    ...
    me.commands[thePlugins] = {
        execCommand: function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialog/text.html',
    ...
    me.addListener('mouseover', function (t, evt) {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', '')) {
            ...
            //var html = popup.formatHtml('<nobr>文本框: <span onclick=$$._edittext() class="edui-clickable"></span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html) {
...
UE.plugins['formtextarea'] = function () {
    ...
    me.addListener('mouseover', function (t, evt) {
        ...
        if (/textarea/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', '')) {
            var html = popup.formatHtml('<nobr>文本域: <span onclick=$$._edittext() class="edui-clickable"></span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html) {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
                popup.showAnchor(popup.anchorEl);
...
//html编辑器
UE.plugins['formricheditor111'] = function ()
{
    var me = this, thePlugins = 'formricheditor';
    me.commands[thePlugins] = {
        execCommand: function ()
        {
            var dialog = new UE.ui.Dialog({
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/textarea/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>HTML编辑器: <span onclick=$$._edittext() class="edui-clickable"></span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
...
UE.plugins['formradio'] = function ()
{
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>单选按钮组: <span onclick=$$._edittext() class="edui-clickable"></span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
...
UE.plugins['formcheckbox'] = function ()
{
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>复选按钮组: <span onclick=$$._edittext() class="edui-clickable"></span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
...
UE.plugins['formdatetime'] = function ()
{
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>日期时间选择: <span onclick=$$._edittext() class="edui-clickable"></span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
...
UE.plugins['formhidden'] = function ()
{
    ...
    me.commands[thePlugins] = {
        ...
        {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialog/hidden.html',
                name: thePlugins + '_' + (new Date().valueOf()),
                editor: this,
                title: '隐藏域',
                cssRules: "width:600px;height:280px;",
...
UE.plugins['formselect'] = function ()
{
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>下拉列表框: <span onclick=$$._edittext() class="edui-clickable"> </span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
...
//自定义选择器
UE.plugins['formselector'] = function () {
    var me = this, thePlugins = 'formselector';
    me.commands[thePlugins] = {
        execCommand: function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialogs/select.aspx',
    ...
    me.addListener('mouseover', function (t, evt) {
        ...
        var type1 = el.getAttribute('type1');
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', '')) {
            var html = popup.formatHtml('<nobr>自定义选择器: <span onclick=$$._edittext() class="edui-clickable"> </span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html) {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
                popup.showAnchor(popup.anchorEl);
...
//自定义选择对话框
UE.plugins['formcustomdialog'] = function () {
    var me = this, thePlugins = 'customdialog';
    me.commands[thePlugins] = {
        execCommand: function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialogs/select.aspx',
    ...
    me.addListener('mouseover', function (t, evt) {
        ...
        var type1 = el.getAttribute('type1');
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', '')) {
            var html = popup.formatHtml('<nobr>自定选择对话框: <span onclick=$$._edittext() class="edui-clickable"> </span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html) {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
                popup.showAnchor(popup.anchorEl);
...
UE.plugins['formfiles'] = function ()
{
    ...
    me.commands[thePlugins] = {
        ...
        {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialog/files.html',
                name: thePlugins + '_' + (new Date().valueOf()),
                editor: this,
                title: '附件上传',
                cssRules: "width:600px;height:280px;",
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>附件上传: <span onclick=$$._edittext() class="edui-clickable"></span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
...
UE.plugins['formgrid'] = function ()
{
    ...
    me.addListener('mouseover', function (t, evt)
    {
        ...
        if (/input/ig.test(el.tagName) && type1 == "flow_" + thePlugins.replace('form', ''))
        {
            var html = popup.formatHtml('<nobr>数据表格: <span onclick=$$._edittext() class="edui-clickable"></span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html)
            {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
    ...
    me.commands[thePlugins] = {
        execCommand: function () {
            var dialog = new UE.ui.Dialog({
                ...
                editor: this,
                title: '新建表单',
                cssRules: "width:400px;height:330px;",
                buttons: [
				{
				    className: 'edui-okbutton',
				    label: '确定',
...
//表单表格
UE.plugins['formtable'] = function () {
    var me = this, thePlugins = 'formtable';
    me.commands[thePlugins] = {
        execCommand: function () {
            var dialog = new UE.ui.Dialog({
                iframeUrl: this.options.UEDITOR_HOME_URL + 'plugins/dialogs/formtable.aspx',
                name: thePlugins + '_' + (new Date().valueOf()),
                editor: this,
                title: '表单表格',
                cssRules: "width:600px;height:300px;",
    ...
    me.addListener('mouseover', function (t, evt) {
        ...
        if (type1 == "flow_" + thePlugins.replace('form', '')) {
            var html = popup.formatHtml('<nobr>表单表格: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>');
            if (html) {
                popup.getDom('content').innerHTML = html;
                popup.anchorEl = el;
                popup.showAnchor(popup.anchorEl);
