//初始化绑定字段选择项
function biddingFileds(filedsJson, textid, $bindfiled)
{
    if (filedsJson && filedsJson.length>0)
    {
        var fieldvalue = "";
        textid = textid || "";
        var opts = getFieldsOps(filedsJson,  textid);
        $bindfiled.html(opts);
    }
}
//得到一连接的所有表
...
//得到一个表所有字段
function getFields(viewTable)
{
    var fields =  _ajaxJson(FUI.Handlers.SmartTableHandler, "GetTableFileds", { viewTable: viewTable });
    return fields;
}

//得到表下拉选择项
function getTableOps(viewTable)
{
...
//得到字段下拉选择项
function getFieldsOps(filedsJson, field)
{
    var fields = filedsJson;
    var options = '<option value=""></option>';
    for (var i = 0; i < fields.length; i++)
    {
        options += '<option value="' + fields[i].field + '" ' + (fields[i].field == field ? 'selected="selected"' : '') + '>' + fields[i].field + (fields[i].title ? '[' + fields[i].title + ']' : '') + '</option>';
    }
    return options;
}
//测试sql合法性
