  ...
  <div class="f2bpm-page-layout costListClass">
    <el-container>
      <el-main class="f2bpm-page-right">
        ...
        <!-- <searchFrom :formSupplierdata='formSupplierdata' @search="searchFormBtnclick" ref="businessFormRef" @exportExcel="exportExcel"></searchFrom> -->
...
<script>
  ...
  import {getlaborList, deleteLabor, exportLabor} from '@/Api/supplier/index'; 
  export default {
    components:{
    ...
    watch:{
      formSupplierdata:{
        handler(oldVal,newValue){
          this.formSupplierdata = newValue
        },
        deep:true
      },
      '$route': function(to, from) {    // 保存成功后返回列表，需要自动展示最新的数据
    ...
    methods: {
      ...
      myOperationDeleteEvent (value, row, index) {
        ...
        FUI.Window.confirm(that, '您确定删除所选记录吗？', null, async function(r) {
          const res = await deleteLabor(row.id);
          if (res.code == 200) {
      ...
      showDialogEdit (formAction, id) {
        ...
        var that = this;
        let queryData = {
          code:'supplierLaborSubcontractExtend',
          path:'/supplierLaborSubcontractExtend/supplierLaborList',
          encode:'supplierLaborSubcontractExtend',
          myId: id || '',
          rowId: id || '',
          modelName:'supplier',  // modelName对应接口前缀
      ...
      toolbarButtonClickEvent ({btn}, event) {
        ...
        if (code == 'closeEdit') {
          ...
          let queryData = {
            code:'supplierLaborSubcontractExtend',
            path:'/supplierLaborSubcontractExtend/supplierLaborList',   // 关闭时回到列表页面
            encode:'supplierLaborSubcontractExtend',
            myId: '',
            rowId: '',
            modelName:'supplier',
      ...
      async getGridData () {
        ...
        // parm.natureOfSupplier = that.formData.natureOfSupplier || null;  // 转换供应商性质natureOfSupplier
        // parm.order = that.tablePage.order; 
        // 调用请求api
        var res = await getlaborList(parm);
        if (res.code === 200) {
      ...
      handelDelete(){
        ...
        FUI.Window.confirm(that, '您确定删除所选记录吗？', null, async function(r) {
          const res = await deleteLabor(arr);
          if (res.code == 200) {
            FUI.Window.showMsg2('删除成功', null, 1);
            that.resetForm()
          } else {
