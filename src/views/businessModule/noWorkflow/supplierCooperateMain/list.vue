  ...
  <div class="f2bpm-page-layout costListClass">
    <el-container>
      <el-main class="f2bpm-page-right">
        <searchFrom ref="businessFormRef" :formData='formData' @search="searchFormBtnclick"></searchFrom>
        <workTable ref="grid"
          grid="grid"
...
<script>
  ...
  import { getSupplierCooperateMain } from '@/Api/supplier/index';
  export default {
    components:{
    ...
    created () {
      ...
      that.tableColumn = [
        ...
        },
        {
          ...
          slots: {
            default: ({ row, column }, h) => {
              ...
              const item1 = {
                ...
                class: "fa fa-cube",
                code: "default.remove",
                click: () => this.toPath(row,'contractDeliverBillMain'),
              };
              const item2 = {
                index: 4,
                title: "去对账",
          ...
          field: "contractOutMainCode",
          title: "主合同编号",
          align: "center",
          type: "html",
        }, {
          field: "contractOutMainName",
          title: "主合同名称",
          align: "center",
          type: "html",
        }, {
    ...
    methods: {
      ...
      toPath(row,type) {
        if(type == 'contractDeliverBillMain') { // 送货单
          this.$router.push('/workflowForm?appId=DF&encode=contractDeliverBillMain&formAction=0&code=custGrid_BusinessSearch_SongHuoDan&modelName=contract&supplierId='+row.id);
        } else { // 对账单
          this.$router.push('/workflowForm?appId=DG&encode=contractReconcileBillMain&formAction=0&code=custGrid_BusinessSearch_DuiZhangDan&modelName=contract&supplierId='+row.id);
        }
      },
      // 操作列查看按钮事件
      ...
      async getGridData () {
        ...
        // 调用请求api
        var res = await getSupplierCooperateMain(parm);
        if (res.code === 200) {
