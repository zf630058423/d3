  ...
  <div class="detail-list">
    <el-form ref="subFrom" :model="subFormData" :rules="rules">
      <div class="detail-list_btns">
        ...
        <el-button type="primary" size="mini" v-show="!readOnly" icon="el-icon-edit-outline" @click="importList">导入</el-button>
      ...
      @selection-change="handleSelectionChange">
        ...
        <el-table-column label="货物或应税劳务、服务名称" prop="nameGoodsTaxableService" width="200" :align="alignCenter">
          <template slot-scope="scope">
           <text-input v-model="scope.row.nameGoodsTaxableService" :disabled="getDisFiled('nameGoodsTaxableService')"></text-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="数量" prop="objectNumber" :align="alignCenter">
          <template slot-scope="scope">
        ...
        <el-table-column label="单价(无税)" prop="unitPriceUntax" :align="alignCenter">
          <template slot-scope="scope">
            <!-- 保留16小数 -->
        ...
        <el-table-column label="金额(含税)" prop="amountTax" :align="alignCenter">
          <template slot-scope="scope">
            <!-- 保留两位小数 -->
        ...
        <el-table-column label="金额(无税)" prop="amountUntax" :align="alignCenter">
          <template slot-scope="scope">
...
export default {
  ...
  data() { 
    return {
      ...
      disabledFiled: {
        'nameGoodsTaxableService': false, //货物或应税劳务、服务名称
  ...
  methods: {
    ...
    validateForm() {
      ...
      //   console.log(_row, 112)
        ...
        for (let key in validObj) {
          if (!_row[key] && key === 'nameGoodsTaxableService') {
    ...
    currentCalculateAmount(key) { 
      ...
      let taxRate = this.taxRateSummar(obj.taxRate); //税率
    ...
    taxRateSummar(tax) { 
      let taxRate = 0;
      if (tax && tax.indexOf("%")) { 
        taxRate = MyFun.getNumDiv(tax.replace("%", ''), '100');
      }
      return taxRate;
    },
    //合计
    getSummaries(param) { 
      ...
      columns.forEach((column, index) => {
        ...
        let columArray = ['unitPriceUntax','amountTax','tax','amountUntax'];
    ...
    async exportList() {
      ...
      await exportApi(this.rowData.modelName, this.rowData.enCode,dataParams)
    },
    //导入
    importList() {
