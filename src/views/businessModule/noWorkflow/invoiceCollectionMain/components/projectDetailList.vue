  ...
  <div class="detail-list">
    <el-form ref="subFrom" :model="subFormData" :rules="rules">
      <div class="detail-list_btns">
        <el-button type="primary" size="mini" icon="el-icon-edit" @click="projectExport">导出</el-button>
      </div>
      <el-table 
      ...
      :summary-method="getSummaries">
        ...
        <el-table-column label="项目编号" prop="constructArchiveMainCode" width="200" :align="alignCenter">
          <template slot-scope="scope">
            <text-input v-model="scope.row.constructArchiveMainCode" :disabled="getDisFiled('constructArchiveMainCode')"></text-input>
          </template>
        </el-table-column>
        <el-table-column label="项目名称" prop="constructArchiveMainName" :align="alignCenter" width="180">
          <template slot-scope="scope">
        ...
        <el-table-column label="支出合同编号" prop="contractOutMainCode" :align="alignCenter">
          <template slot-scope="scope">
            <text-input v-model="scope.row.contractOutMainCode" :disabled="getDisFiled('contractOutMainCode')"></text-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="认领金额(含税)(元)" prop="claimAmountTax" :align="alignCenter">
          <template slot-scope="scope">
            <amount-input v-model="scope.row.claimAmountTax" :disabled="getDisFiled('claimAmountTax')"></amount-input>
          </template>
        </el-table-column>
        <el-table-column label="认领税额(元)" prop="claimTaxAmount" :align="alignCenter">
          <template slot-scope="scope">
            <amount-input v-model="scope.row.claimTaxAmount" :disabled="getDisFiled('claimTaxAmount')"></amount-input>
          </template>
        </el-table-column>
        <el-table-column label="管理部门" prop="constructionUnitName" :align="alignCenter">
          <template slot-scope="scope">
            <text-input v-model="scope.row.constructionUnitName" :disabled="getDisFiled('constructionUnitName')"></text-input>
          </template>
        </el-table-column>
        <el-table-column label="项目部" prop="projectDepartmentName" :align="alignCenter">
          <template slot-scope="scope">
            <text-input v-model="scope.row.projectDepartmentName" :disabled="getDisFiled('projectDepartmentName')"></text-input>
          </template>
        </el-table-column>
      </el-table>
...
import { exportDataProjectApi } from '@/Api/invoice/invoiceApplyApi';
  ...
  data() { 
    return {
      ...
      disabledFiled: {
        ...
        'claimAmountTax': false, //认领金额(含税)(元)
        'claimTaxAmount': false,  //认领税额(元)	
        'constructionUnitName': false,  //管理部门
        'projectDepartmentName': false,  //项目部
      },
