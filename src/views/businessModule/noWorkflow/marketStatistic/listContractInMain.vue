<template>
  <!-- 跨区涉税台账 -->
  <div class="f2bpm-page-layout">
    <el-container>
      <el-main class="f2bpm-page-right">
        <div class="search-box" ref="contractForm">
          <contractForm @search="searchFn" ref="searchFormRef"></contractForm>
        </div>
        <div class="search-btns">
          <el-button type="primary" icon="el-icon-edit-outline" size="small" class="fa"
            @click="exportExcel">导出</el-button>
        </div>
        <!--  -->
        <el-table v-loading="loading" ref="gridTable" :border="true" stripe row-key="id" :data="tableData"
          :columns="tableColumn" :height="gridHeight" show-summary style="width: 100%">
          <template v-for="column in tableColumn">
            <el-table-column :prop="column.field" :label="column.title" :width="column.width" :minWidth="column.minWidth"
              :key="column.field" :showOverflowTooltip="column.showOverflowTooltip" :align="column.align"
              :type="column.type" :sortable="column.sortable"></el-table-column>
          </template>
        </el-table>
        <el-pagination background align="center" :layout="tablePage.layout" :total="tablePage.total"
          :current-page="tablePage.currentPage" :page-size="tablePage.pageSize" @size-change="pageSizeChange"
          @current-change="pageChange"></el-pagination>
      </el-main>
    </el-container>
  </div>
</template>
<script>
import contractForm from './components/contractForm.vue';
import { getContractInMain, exportContractData } from '@/Api/voucher.js';

export default {
  components: { contractForm },
  data () {
    return {
      tablePage: {
        align: "center",
        currentPage: 1,
        layout: "total, sizes, prev, pager, next, jumper",
        order: "",
        pageCount: 10,
        total: 50,
        pageSize: 20,
        pageSizes: [10, 15, 20, 25, 50, 100, 200, 500]
      },
      // 列表数据源
      tableData: [],
      tableColumn: [
        {
          type: "selection",
          field: "selection",
          align: "center",
          width: 55,
        },
        {
          type: "index",
          field: "index",
          title: "序号",
          align: "center",
          width: 55,
        },
        {
          field: "constructCode",
          title: "项目编号",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "constructName",
          title: "项目名称",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "businessUnit",
          title: "业务单元",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "projectDepartmentName",
          title: "项目部",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "contractCode",
          title: "合同编号",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "contract",
          title: "合同名称",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "contractSignDate",
          title: "合同签订日期",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
          sortable: true,
        },
        {
          field: "contStartDate",
          title: "合同开始日期",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
          sortable: true,
        },
        {
          field: "contEndDate",
          title: "合同竣工/结束日期",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "customerA",
          title: "甲方",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "customerB",
          title: "甲方（二）",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "customerC",
          title: "甲方（三）",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "supplierName",
          title: "乙方",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "norigsignmnyTotalTax",
          title: "合同总金额(元)",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "norigsignmnyMsterTotalTax",
          title: "主合同金额(元)",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "addAmountTotalTax",
          title: "增补合同金额(元)",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "changeAmountTax",
          title: "变更金额(元)",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "authorizeTotalAmount",
          title: "审定金额(元)",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "invoiceAmount",
          title: "开票金额(元)",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "schemeCode",
          title: "项目协议编号",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "scheme",
          title: "项目协议名称",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "createUserName",
          title: "合同编制人",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "createTime",
          title: "合同编制时间",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
        {
          field: "passTime",
          title: "合同审批通过时间",
          align: "center",
          minWidth: 130,
          showOverflowTooltip: true,
        },
      ], // 表头字段配置
      pageData: {
        currentPage: 1,
        pageSize: 20,
        isPaging: 1
      },
      formData: {},
      loading: false,
      marketCenterArray: [],  //业务中心合并单元格数组
      marketUnitArray: [],  //业务单元合并单元格数组
      gridHeight: 750,
      mergeObj: {},
      mergeArr: ['marketUnit', 'marketCenter'],
    }
  },
  mounted () {
    // window.myvm指向当前页面vue实例
    window.myvm = this;
    // 自适应计算高度
    this.autoPageHeight();
  },
  created () {
    this.getDataList();
  },
  methods: {
    autoPageHeight () {
      const cHeight = document.body.clientHeight;
      const contractFormHeight = this.$refs.contractForm.clientHeight;
      this.gridHeight = (cHeight - contractFormHeight) + 'px';
    },
    async searchFn (obj) {
      this.pageData.currentPage = 1;
      this.pageData.pageSize = 20;
      this.formData = obj;
      this.getDataList();
    },
    async getDataList () {
      let dataParams = {
        ...this.pageData,
        ...this.formData
      }
      try {
        this.loading = true;
        let { data } = await getContractInMain(dataParams);
        let { pagination, list } = data
        this.loading = false;
        this.tableData = list;
        this.tablePage.currentPage = pagination.currentPage;
        this.tablePage.total = pagination.total;
        this.autoPageHeight();
      } catch (error) {
        this.loading = false;
      }
    },
    async exportExcel () {
      let param = this.$refs.searchFormRef.paramsData() || {};
      await exportContractData(param);
    },
    pageSizeChange (pageSize) {
      this.pageData.pageSize = pageSize;
      this.getDataList();
    },
    pageChange (page) {
      this.pageData.currentPage = page;
      this.getDataList();
    },
  }
}
</script>
<style scoped lang="scss">
.f2bpm-page-layout {
  .search-btns {
    padding: 0 16px;
    margin-bottom: 8px;
  }
}
</style>