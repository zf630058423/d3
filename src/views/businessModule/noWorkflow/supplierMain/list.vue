...
<script>
  ...
  // import {getSporadiclist, deleteSporadic} from '@/Api/customerSupplierList/index';
  import {getDataDictByConfigType} from '@/Api/dataDict.js'
  import { getSporadiclist, deleteSporadic, exportSporadic } from '@/Api/supplier/index';
  export default {
    components:{
    ...
    methods: {
      ...
      myOperationDeleteEvent (value, row, index) {
        ...
        FUI.Window.confirm(that, '您确定删除所选记录吗？', null, async function(r) {
          const res = await deleteSporadic(row.id);
          if (res.code == 200) {
      ...
      showDialogEdit (formAction, id) {
        ...
        let queryData = {
          code:'supplierMain',
          path:'/supplierMain/sporadicList',  // sporadicList sporadicForm
          encode:'supplierMain',
          myId: id || '',
          rowId: id || '',
      ...
      async getGridData () {
        ...
        // 调用请求api
        var res = await getSporadiclist(parm);
        if (res.code === 200) {
      ...
      handelDelete(){
        ...
        FUI.Window.confirm(that, '您确定删除所选记录吗？', null, async function(r) {
          const res = await deleteSporadic(arr);
          if (res.code == 200) {
            FUI.Window.showMsg2('删除成功', null, 1);
            that.resetForm()
          } else {
      ...
      async exportExcel(obj) { 
        ...
        this.loading = true;
        await exportSporadic(paramData);
        this.loading = false;
      }
    }
  }
