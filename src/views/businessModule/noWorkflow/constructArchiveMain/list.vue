    ...
    <el-container>
      <el-main class="f2bpm-page-right">
        <div class="search-box">
          <SearchForm ref="form" @searchForm-click="searchFormBtnclick"></SearchForm>
        </div>
        <workTable ref="grid"
                   grid="grid"
...
<script>
  ...
  import {getList,deleteForm} from '@/Api/constructArchive/index';
  import { exportData } from '@/Api/workflowBusssion/index.js'
  import {getDataDictByConfigType} from '@/Api/dataDict.js'
  export default {
    components:{
      workTable,SearchForm
    },
    data () {
      return {
        ...
        isOpen: false,
        value1: null,
        itemTypeList: [], // 立项类型list
        projectStateList: [], // 项目状态list
    ...
    created () {
      ...
      that.tableColumn = [
        ...
        },
        {
          ...
          slots: {
            default: ({ row, column }, h) => {
              return (this.getCustomerStatus(row.projectStatusId,0))
            },
          }
        },{
          field: "customerName",
          ...
          slots: {
            default: ({ row, column }, h) => {
              return (this.getCustomerStatus(row.itemTypeId,1))
            },
          }
        }, {
          field: "createTime",
    ...
    methods: {
      ...
      showDialogEdit (formAction, id) {
        var that = this;
        let queryData = {
          code:'constructArchiveMain',
          path:'constructArchiveMain/list',
          encode:'constructArchiveMain',
          myId:id || '',
          modelName:'construct',
      ...
      myToolBarCustomButton () {
        var btns = [
          ...
          { code: 'sys_excelExport', status: 'primary', name: '导出', icon: 'icon icon-daochu' }
        ];
      ...
      toolbarButtonClickEvent ({btn}, event) {
        ...
        if (code == 'closeEdit') {
          ...
          let url = '/constructArchive/form?formAction=0';
          ...
          // 统一的导出事件
          that.exportCommonExcel('customDataGrid', '');
        }else {
          var fn = that[code];
          fn(code);
        }
      ...
      async getDataDictByConfigType() {
        const res = await getDataDictByConfigType({ params: { configTypes: '项目状态,立项类型' }});
        if (res.success) {
          res.data.forEach(ele => {
            switch (ele.dataDictType) {
              case '项目状态':
                this.projectStateList = ele.dataDict;
                break;
                case '立项类型':
      ...
      getCustomerStatus(status,oIndex) {
        let text;
        if(oIndex == 0){
          var array = this.projectStateList;
          for(let i in array){
            if(array[i].configValue == status){
              text = array[i].configName
            }

          }
        }else if(oIndex == 1){
          var array = this.itemTypeList;
          for(let i in array){
            if(array[i].configValue == status){
              text = array[i].configName
            }
      ...
      async exportCommonExcel(type, tableName) {
        var that = this;
        var idFieldValues = [];
        var arr = that.multipleSelection; // 勾选中的数据
        for(let i in arr){
          idFieldValues.push(arr[i].id);
        }
        var selectedIds = idFieldValues ? idFieldValues : [];
        ...
        parm.sort = that.tablePage.sort;
        parm.order = that.tablePage.order;
        var modelName = 'construct';
        var keyWord = 'constructArchiveMain';
        await exportData(modelName, keyWord, parm)
      },
    }
  }
</script>
