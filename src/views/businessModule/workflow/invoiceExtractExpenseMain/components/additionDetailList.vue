  ...
  <div class="detail-list">
    <el-form ref="subFrom" :model="subFormData">
      ...
      :summary-method="getSummaries">
        ...
        <el-table-column label="税种" prop="taxName" width="200" :align="alignCenter">
          <template slot-scope="scope">
           <text-input v-model="scope.row.taxName" :disabled="getDisFiled('taxName')"></text-input>
          </template>
        </el-table-column>
        <el-table-column label="税基(元)" prop="taxBase" :align="alignCenter" width="180">
          <template slot-scope="scope">
            <text-input v-model="scope.row.taxBase" :disabled="getDisFiled('taxBase')"></text-input>
          </template>
        </el-table-column>
        <el-table-column label="税率" prop="taxRate" :align="alignCenter">
          <template slot-scope="scope">
        ...
        <el-table-column label="缴纳税金" prop="taxes" :align="alignCenter">
          <template slot-scope="scope">
            <text-input v-model="scope.row.taxes" :disabled="getDisFiled('taxes')"></text-input>
          </template>
        </el-table-column>
        <el-table-column label="OCR税金" prop="ocrTaxes" :align="alignCenter">
          <template slot-scope="scope">
            <text-input v-model="scope.row.ocrTaxes" :disabled="getDisFiled('ocrTaxes')"></text-input>
          </template>
        </el-table-column>
      ...
      handler:function(val) {
        if (val) { 
          this.keepData = (val && val.length > 0) ? val.map((item,index) => ({
            ...
            taxBase: this.judageString(item.taxBase),  //税基(元)
            taxes: this.judageString(item.taxes),  //缴纳税金
            ocrTaxes: this.judageString(item.ocrTaxes),  //OCR税金
          })) : [];
          if (this.readOnly) {
  ...
  methods: {
    ...
    getSummaries(param) { 
      ...
      columns.forEach((column, index) => {
        ...
        //发票金额含税合计
        if (column.property === 'taxes') {
          this.$emit('taxesChange', sums[index]);
        }
      });
