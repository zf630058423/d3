      ...
      <el-table :data="subFormData.detailList" style="width: 100%" size="small" fit class="f2bpmui-datagrid-vuetable"
        ...
        <el-table-column label="收款方式">
          <template slot-scope="scope">
            <el-form-item>
              <el-select v-model="scope.row.term" placeholder="请选择" size="small" style="width: 100%">
        ...
        <el-table-column label="收款比例（%）">
          <template slot-scope="scope">
            <el-form-item :prop="`detailList.${scope.$index}.scale`" :rules="rules.scale">
              <!-- <el-input type="text" v-model="scope.row.scale" size="small" placeholder="请输入大于 0 小于 1 的两位小数，总比例为100%"
                clearable @focus="focusInput('scale', scope.$index)" /> -->

                <percentage-input v-model="scope.row.scale" clearable></percentage-input>
            </el-form-item>
          </template>
        </el-table-column>
...
export default {
  ...
  updated () {
    ...
    if (rowId && this.subFormData.detailList[0].scale && this.isValidate) {
  ...
  methods: {
    validateNumber (rule, value, callback) {
      ...
      const pattern = /^(?:0|[1-9][0-9]?|100)(\.[0-9]{0,2})?$/
      console.log(value)
      ...
      // this.subFormData[field[0]][field[1]][field[2]] = parseInt(this.toNumber2(value) * 100) + '%'
      callback()
    },
    ...
    validateForm () {
      ...
      const _keepData = Object.toDeepClone(this.keepData).map(item => {
        item.scale = this.toNumber2(item.scale)
        return item
      })
      for (let index = 0; index < _keepData.length; index++) {
        let _row = _keepData[index]
        let _currentPage = parseInt(index + 1 / _pageSize) + 1
        sum = sum + this.toNumber2(_row.scale)
        if (index === _keepData.length - 1 && sum != 1) {
          this.$message.error(`收款方式中收款比例的总和应该达到100%`)
          return false
        }
        for (let key in validObj) {
          if (!_row[key]) {
            this.$message.error(`收款方式：第 ${_currentPage} 页，第 ${(index + 1) % _pageSize} 行 ${validObj[key]} 不能为空`)
            return false
          }
        }
      }
