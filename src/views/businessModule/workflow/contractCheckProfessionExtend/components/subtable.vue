    ...
    <div class="f2bpm-online-subtable-divcontainer">
      <el-table :key="subtableKey" :data="tempRows" class="f2bpmui-datagrid-vuetable" border show-summary  :summary-method="getSummaries1">
        ...
        <el-table-column label="说明">
          <template slot-scope="scope">
            <el-input v-model="scope.row.memo" size="small" placeholder="自动带出"/>
          </template>
        </el-table-column>
        ...
        <el-table-column label="本次核量">
          <div slot="header"><span style="color: #ed4014">*</span> 本次核量</div>
          <template slot-scope="scope">
        ...
        <el-table-column label="累计核量">
          <template slot-scope="scope">
            <el-input type="text" v-model="scope.row.theCumulativeNuclear" size="small" placeholder="自动计算" disabled/>
          </template>
        </el-table-column>
        <el-table-column label="上期累计核量">
          <template slot-scope="scope">
            <el-input v-model="scope.row.accumulativeQuantity" size="small" placeholder="自动计算" disabled/>
          </template>
        </el-table-column>
        <el-table-column label="单价(含税)">
          <template slot-scope="scope">
            <amount-input v-model="scope.row.unitPriceTax" size="small" disabled placeholder="自动带出" />
          </template>
        </el-table-column>
...
import {getContractAmountInfo,getContractCheckLaborTeamExtendDetail} from '@/Api/contract/index.js'
  ...
  methods: {
    ...
    changeAmountCount(index){
      ...
      var that = this;
      var num = 0; // 本次核量
      var theOfNuclear = that.tempRows[index]['theOfNuclear'];// 本次核量
      var theCumulativeNuclear = that.tempRows[index]['theCumulativeNuclear'];// 本次核量
      num = Number(theOfNuclear)+Number(theCumulativeNuclear);
      num = Object.tofixed(num,4);
      that.$set(that.tempRows[index],'theCumulativeNuclear',num);
    ...
    async getTeamContractDetailFn(contractOutMainId){
      var that = this;
      let res = await getContractCheckLaborTeamExtendDetail(contractOutMainId);
      if(res.code === 200){
        var list = res.data;
        if (list && list.length > 0) {
          $.each(list, function (index, row) {
            row.tempid = 'temp_' + Object.toGuid();
            row.accumulativeQuantity = row.accumulativeQuantity?row.accumulativeQuantity:'';
      ...
      for(let i=0;i<list.length;i++){
        ...
        if(Number(number) > 0){
          if(Number(theOfNuclear) < 0){
            ...
            FUI.Window.showMsg2('第' + page + '页第' + rowNum + ' 累计核量＞合同数量，请走 合同增补流程后再提交此流程！');
            return false
          }
        }
      }
