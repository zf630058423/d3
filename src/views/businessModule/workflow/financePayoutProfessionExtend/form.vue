...
<template>
  <div v-loading="loading">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" :disabled="setting.readonly" label-width="160px"
      ...
      <row-layout :cols="3">
        <el-form-item slot="col_1" label="累计核量金额(含税)(元)：">
          <el-input v-model="dataForm.cumulativeAuditAmountTax" size="small" placeholder="自动填写" disabled/>
        </el-form-item>
        <el-form-item slot="col_2" label="结算金额(含税)(元)：">
          <el-input v-model="dataForm.settlementAmount" size="small" placeholder="自动填写" disabled/>
        </el-form-item>
        <el-form-item slot="col_3" label="应付金额(含税)(元)：">
          <el-input v-model="dataForm.amountPayable" disabled size="small" placeholder="自动计算"/>
        </el-form-item>
      </row-layout>
      <row-layout :cols="3">
        <el-form-item slot="col_1" label="付款性质：" prop="prepay">
          <el-select v-model="dataForm.prepay" placeholder="请选择" size="small" @change="changePrepay">
            <el-option v-for="(item,index) in prepayOptions" :key="index" :label="item.configName" :value="item.configValue">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item slot="col_2" label="本次申请金额(元)：" prop="applyAmount">
          <el-input v-model="dataForm.applyAmount" size="small" placeholder="自动计算"/>
        </el-form-item>
        <el-form-item slot="col_3" label="结算方式：" prop="balaType">
      ...
      <row-layout :cols="3">
        <el-form-item slot="col_1" label="票据号：" :prop="setRules('checkNo')" v-if="['10','13','9','21','14'].indexOf(dataForm.balaType) != -1?true:false">
          <el-input v-model="dataForm.checkNo" size="small" placeholder="自动带出"
                    :disabled="setRules('checkNo')?false:true"/>
        </el-form-item>
        <el-form-item slot="col_2" label="供应商：">
          <el-input v-model="dataForm.supplierName" size="small" placeholder="自动带出" disabled/>
        </el-form-item>
        <el-form-item slot="col_3" label="收款账号：" :prop="dataForm.balaType == 1?'payeeBankAcc':''">
      ...
      <row-layout :cols="3">
        <el-form-item slot="col_1" label="收款银行：">
          <amount-input v-model="dataForm.payeeBank" size="small" placeholder="自动带出" disabled/>
        </el-form-item>
        <el-form-item slot="col_2" label="收款银行联行号：">
          <amount-input v-model="dataForm.interBankNo" size="small" placeholder="自动带出" disabled/>
        </el-form-item>
        <el-form-item slot="col_3" label="退款金额(元)：">
          <amount-input v-model="dataForm.refundAmount" size="small" placeholder="自动带出" disabled/>
        </el-form-item>
      </row-layout>
      <row-layout :cols="3">
      ...
      <row-layout :cols="1">
        <el-form-item slot="col_1" label="备注：">
          <el-input v-model="dataForm.remark" type="textarea" :autosize="{ minRows: 5}" size="small" placeholder="请输入，内容长度不超过250个字符"
                    maxlength="250"/>
        </el-form-item>
      </row-layout>
      ...
      <el-table :data="dataForm.invoiceCollectionPaymentDetail" fit class="f2bpmui-datagrid-vuetable" border
        ...
        <el-table-column label="操作" width="160px" align="center">
          <template slot-scope="scope">
            <el-button type="text" icon="el-icon-link">超链接</el-button>
...
<script>
  ...
  import {getPayoutProfessionAmount} from '@/Api/contract/index.js'
  ...
  const requiredArray = ['fileInfoList','applyAmount', 'checkNo', 'payeeBankAcc', 'paymentOrgName','prepay', 'constructArchiveMainCode', 'isPaymentByPartyA', 'contractOutMainCode', 'applyAmount', 'paymentType', 'balaType', 'paymentAccountNumber', 'invoiceSituation', 'contractTeamPayrollCheckMainNo'];
  export default {
    components: {
      FormTitle,
      selectorDialog,
    ...
    data() {
      return {
        ...
        dataForm: {
          ...
          contractTeamPayrollCheckMainNo: '',
          createTime: '',
          createUserId: '',
          createUserName: '',
        ...
        baseBankDialog: PAYEEBANK_MODEL,
        ...
        invoiceDialog: {
          ...
          isMulti: false,
          initailData: [],
          parm: {varParams: [{key: 'constructArchiveMainId', value: ''},{key: 'contractOutMainId', value: ''}],singleSelect:true,moduleType:'2'} //moduleType： 1-劳务付款；2-专业分包付款；3-材料付款；4-租赁付款；5-其他付款；6-项目费用报销；7-员工费用报销；8-集采付款；
        },
    ...
    methods: {
      ...
      updateMasterContractValue(data) {
        ...
        const {constructarchivemainid, constructarchivemaincode, constructarchivemainname, projectmanagername, constructionunitname,projectManagerId,constructionUnitId} = data[0];
        ...
        this.setlinkUrl(constructarchivemainid);
      },
      /**
       * 主合同名称(控件)
       */
      updateMainContractValue(data) {
        ...
        this.getPayoutProfessionAmountFn()
        this.$forceUpdate()
      },
      /**
       * 付款财务组织
      ...
      afterInit() {
        var that = this;
        var val1 = that.dataForm.noAppointSupplier;
         val1 = val1?(val1).toString():'';
        var val2 = that.dataForm.isUsedGmAttendanceMachine;
        val2 = val2?(val2).toString():'';
        var val3 = that.dataForm.isPaymentByPartyA;
        ...
        if (val3) {
          that.dataForm.isPaymentByPartyA = val3;
        }
        if(that.dataForm.constructArchiveMainId){
          that.setlinkUrl(that.dataForm.constructArchiveMainId)
        }
      },
      /**
       * 获取表单数据
      ...
      async getDataDictByConfigType() {
        const res = await getDataDictByConfigType({params: {configTypes: '是或否,结算方式,发票类型,发票情况,付款性质'}});
        if (res.success) {
      ...
      vaildTableList() {
        ...
        if (type == 1 && arr.length > 0) {
          for (var j = 0; j < arr.length; j++) {
            if (arr[j]) {
              var fileName = (arr[j].fileName).toString(); //发票号码
              if (fileName) {
                return !isOk;
              } else {
                var msg = '';
      ...
      // 获取应付金额
      async getPayoutProfessionAmountFn(){
        var that = this;
        var parms = {
          "contractOutMainId": that.dataForm.contractOutMainId,
          "financePayoutMainId": that.dataForm.id,
          "prepay": that.dataForm.prepay
        };
        const res = await getPayoutProfessionAmount(parms);
        if(res.code === 200){
          const {amountPayable,cumulativeAuditAmountTax,settlementAmount,totalContractAmount} = res.data;
          that.dataForm.amountPayable = amountPayable?amountPayable:'';
          that.dataForm.settlementAmount = settlementAmount?settlementAmount:'';
          that.dataForm.cumulativeAuditAmountTax = cumulativeAuditAmountTax?cumulativeAuditAmountTax:'';
          that.dataForm.totalContractAmount = totalContractAmount?totalContractAmount:'';
        }
      },
      // 子表数据导入
      handelImportData(){
      ...
      // 超链接拼接方法
      setlinkUrl(id){
        let code = 'constructArchiveMain'; // 菜单code
        let path = 'constructArchiveMain/list'; // 路由地址
        let encode = 'constructArchiveMain';//业务code
        let myId = id;//主表id
        let modelName = 'construct'; // 模块code
        let formAction = 3; // 表单状态 默认 3
        let title= '项目档案-详情';
        let isInterface=true;
        let fullDev=true;
        let url = __webpath+'/noWorkflowForm?code='+code+'&path='+path+'&encode='+encode+'&myId='+myId+'&modelName='+modelName+'&formAction='+formAction+'&title='+title+'&isInterface='+isInterface+'&fullDev='+fullDev
        this.masterContractDialog.linkUrl = url
