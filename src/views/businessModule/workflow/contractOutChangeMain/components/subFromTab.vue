...
export default {
  ...
  data () {
    return {
      ...
      importData: {
        visible: false,
        url: '/contract/contractOutChangeMain/importData',
        key: 'contractOutChangeDetail'
      },
  ...
  methods: {
    ...
    validateForm () {
      ...
      for (let index = 0; index < _keepData.length; index++) {
        ...
        taxAmount = taxAmount + Number(_row.taxAmount)
      }
      this.$emit('changeContractAmount', { contractAmountTax, contractAmountTaxFree, taxAmount })
