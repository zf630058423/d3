<template>
  ...
  <div class="bank-files-form">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="160px" :disabled="setting.readonly">
      ...
      <row-layout :cols="3" title="办理信息" marginStyle="margin: 16px 16px 0 16px">
        <template slot="col_1">
          <el-form-item prop="applicationCode" label="申请单号：">
            <select-dialog :value="dataForm.applicationCode" @updateValue="voucherCrossMainCode"
              :modelData="voucherCrossModelData" dynamicModel="customDialog"></select-dialog>
          </el-form-item>
          <el-form-item prop="handleDate" label="办理日期：">
            <el-date-picker v-model="dataForm.handleDate" value-format="yyyy-MM-dd" type="date"
              placeholder="选择日期时间" :disabled='!isZhuanYuanApprove'></el-date-picker>
          </el-form-item>
          <el-form-item prop="effectiveEndDate" label="有效截止日期：">
        ...
        <template slot="col_2">
          <el-form-item prop="theApplyAmount" label="本次开证金额(元)：">
            <el-input v-model="dataForm.theApplyAmount" size="small" :placeholder="pleaseEnterPlaceHolder" disabled/>
          </el-form-item>
        ...
        <template slot="col_3">
          <el-form-item prop="taxAcross" label="税跨报号：">
            <el-input v-model.number="dataForm.taxAcross" size="small" placeholder="" :disabled='!isZhuanYuanApprove'></el-input>
          </el-form-item>
      ...
      <row-layout :cols="1" :title="'附件'">
        <el-form-item slot="col_1" prop="fileInfoList" label="附件文件：">
          <update-files v-model="dataForm.fileInfoList" :disabled='!isZhuanYuanApprove'></update-files>
        </el-form-item>
      </row-layout>
      <row-layout :cols="3">
        <template slot="col_1">
...
export default {
  name: "voucherCrossTaxReportMain",
  components: {
    RowLayout,
    updateFiles,
    SelectDialog,
  ...
  data () {
    return {
      dataForm: {
        ...
        contractName: "", //收入合同名称
        crossReportState: "5", //税跨报状态
        customerA: "", //甲方
        deptId: "", //部门id
        deptName: "", //部门
        fileInfoList: [], //附件
        invoiceOpenAmountTax: "", //已开票金额(含税)(元)
        handleDate: new Date(), //办理日期
        releaseDate: "", //发证日期
      ...
      voucherCrossModelData: { code: 'KuaQuSheShuiBaoGaoShenQingDanHao', parm: { varParams: [{ key: 'crossReportState', value: '4' },{ key: 'constructArchiveMainId', value: '' }] } }, //跨税报号（通用）申请中
      ...
      dataRule: RuleRequired.dataRule(['contractCode', 'constructCode', 'contactPersonRrossTaxRelated',
  ...
  computed: {
    ...
    /**
         * 只有 税务组、税务会计审批节点 可上传、编辑文件；
         * 且不可为编制人
         */
     isZhuanYuanApprove() {
      const isRequired = ['税务组','园林税务会计','幕墙税务会计','建工税务会计'].find(res=> {return WFContext.CurrentActivityName.indexOf(res) != -1 
        && this.isConstruction})?true:false;
        const arr = ['taxAcross','handleDate','releaseDate','effectiveEndDate','effectiveStartDate','fileInfoList']
        arr.forEach(item=>{
  ...
  methods: {
    ...
    voucherCrossMainCode (data) {
      this.dataForm.applicationCode = data[0].applicationCode;//申请单号
      this.dataForm.applicationId = data[0].applicationId;//申请单号id
      ...
      this.dataForm.theApplyAmount = data[0].theApplyAmount; //开证金额
    },
    async validateForm () {
      const _this = this;
