<template>
  ...
  <div class="bank-files-form">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="160px" :disabled="setting.readonly">
      ...
      <row-layout :cols="3" title="延期信息" marginStyle="margin: 16px 16px 0 16px">
        <template slot="col_1">
          ...
          <el-form-item prop="extendDeadDate" label="延期截止日期：">
            <el-date-picker v-model="dataForm.extendDeadDate" value-format="yyyy-MM-dd" type="date"
        ...
        <template slot="col_2">
          ...
          <el-form-item prop="effectiveStartDate" label="有效开始日期：">
            <el-date-picker v-model="dataForm.effectiveStartDate" value-format="yyyy-MM-dd" type="date" disabled
              placeholder="选择日期时间"></el-date-picker>
          </el-form-item>
        </template>
        <template slot="col_3">
          ...
          <el-form-item prop="effectiveEndDate" label="有效截止日期：">
            <el-date-picker v-model="dataForm.effectiveEndDate" value-format="yyyy-MM-dd" type="date" disabled
              placeholder="选择日期时间"></el-date-picker>
          </el-form-item>
        </template>
      </row-layout>
      <row-layout :cols="1" marginStyle="margin: 0 16px 16px 16px">
        <template slot="col_1">
          <el-form-item prop="delayReason" label="延期原因：">
            <el-input v-model="dataForm.delayReason" type="textarea" :rows="3" :placeholder="autofillPlaceHolder" />
          </el-form-item>
...
export default {
  name: "voucherCrossTaxDelayApplyMain",
  components: {
    RowLayout,
    updateFiles,
    SelectDialog,
  ...
  data () {
    return {
      dataForm: {
        ...
        extendDeadDate: "", //延期截止日期
        effectiveEndDate: "", //最新有效截止日期
      ...
      dataRule: RuleRequired.dataRule(['contractCode', 'constructCode', 'contactPersonRrossTaxRelated',
        'contactInformation', 'isNullifyReopen', 'taxUnitName', 'stampTaxMethodPayment', 'fileInfoList','taxAcross','extendDeadDate','delayReason']),
  ...
  methods: {
    ...
    voucherCrossMainCode (data) {
      ...
      this.dataForm.crossReportState = data[0].crossReportState; //税跨报状态
      this.dataForm.releaseDate = data[0].releaseDate; //发证日期
      this.dataForm.effectiveStartDate = data[0].effectiveStartDate; //有效开始日期
      this.dataForm.effectiveEndDate = data[0].effectiveEndDate; //有效截止日期
    },
    async validateForm () {
      const _this = this;
      if(this.dataForm.effectiveEndDate){
        if(moment(this.dataForm.extendDeadDate).isBefore(moment(this.dataForm.effectiveEndDate))){
          FUI.Window.showMsg2('延期截止日期需大于有效截止日期');
          }
      }
