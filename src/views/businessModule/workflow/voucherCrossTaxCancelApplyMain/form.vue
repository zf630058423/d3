<template>
  ...
  <div class="bank-files-form">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="160px" :disabled="setting.readonly">
      ...
      <row-layout :cols="3" title="作废信息" marginStyle="margin: 16px 16px 0 16px">
        <template slot="col_1">
          ...
          <el-form-item prop="cancelDate" label="作废日期：">
            <el-date-picker v-model="dataForm.cancelDate" value-format="yyyy-MM-dd" type="date"
      ...
      <row-layout :cols="1" marginStyle="margin: 0 16px 16px 16px">
        <template slot="col_1">
          <el-form-item prop="cancelReason" label="作废原因：">
            <el-input v-model="dataForm.cancelReason" type="textarea" :rows="3" :placeholder="autofillPlaceHolder" />
          </el-form-item>
...
export default {
  name: "voucherCrossTaxCancelApplyMain",
  components: {
    RowLayout,
    updateFiles,
    SelectDialog,
  ...
  data () {
    return {
      ...
      voucherCrossModelData: { code: 'KuaQuSheShuiBaoHaoBaoFei', parm: { varParams: [{ key: 'crossReportState', value: '3' }, { key: 'constructArchiveMainId', value: '' }] } }, //跨区涉税开具（报废税号）
      ...
      dataRule: RuleRequired.dataRule(['contractCode', 'constructCode', 'contactPersonRrossTaxRelated',
        'contactInformation', 'isNullifyReopen', 'taxUnitName', 'stampTaxMethodPayment', 'fileInfoList','taxAcross','cancelDate','cancelReason']),
