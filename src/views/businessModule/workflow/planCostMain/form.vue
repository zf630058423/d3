    ...
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" :disabled="setting.readonly" label-width="140px">
      ...
      <row-layout :cols="3">
        ...
        <el-form-item slot="col_2" label="施工金额增补(含税)：">
          <amount-input v-model="dataForm.changeContructionAmountTax" defaultValue clearable disabled
                        :placeholder="msgTip.ENTER_PLACEHOLDER"></amount-input>
        </el-form-item>
      ...
      <row-layout :cols="3">
        ...
        <el-form-item slot="col_2" prop="customerPhone" label="查看最新计划：">
          <el-button type="text">{{planCostLink}}</el-button>
        </el-form-item>
      </row-layout>
      <row-layout :cols="1">
      ...
      <el-table :data="dataForm.planCostSummaryDetailList" class="f2bpmui-datagrid-vuetable" border :key="tableKey">
        ...
        <el-table-column label="目标成本(含税)(元)">
          <template slot-scope="scope">
            <amount-input v-if="scope.row.totalExpenseType == '4' || scope.row.totalExpenseType == '5'" @change="changeTotalTargetAmount1" v-model="scope.row.totalTargetAmountTax" size="small" placeholder="请输入"/>
            <percentage-input v-else-if="scope.row.totalExpenseType == '8'" v-model="scope.row.totalTargetAmountTax" size="small" placeholder="自动计算，可修改"></percentage-input>
            <span v-else>{{(scope.row.totalTargetAmountTax || msgTip.AUTO_PLACEHOLDER)}}</span>
          </template>
        </el-table-column>
        <el-table-column label="目标成本(无税)(元)">
          <template slot-scope="scope">
            <amount-input v-if="scope.row.totalExpenseType == '4' || scope.row.totalExpenseType == '5'" @change="changeTotalTargetAmount2"  v-model="scope.row.totalTargetAmountUntax" size="small" placeholder="请输入"/>
            <percentage-input v-else-if="scope.row.totalExpenseType == '8'" v-model="scope.row.totalTargetAmountUntax" size="small" placeholder="自动计算，可修改"></percentage-input>
            <span v-else-if="scope.row.totalExpenseType == '6'">--</span>
            <span v-else>{{scope.row.totalTargetAmountUntax || msgTip.AUTO_PLACEHOLDER}}</span>
          </template>
        </el-table-column>
      ...
      <el-table :data="dataForm.planCostFeeDetailList" border class="f2bpmui-datagrid-vuetable" >
        <el-table-column label="No." width="100px">
          <template slot-scope="scope">
            <span style="color: #0c0b0e">{{scope.row.serialNumber}}</span>
          </template>
        ...
        <el-table-column label="目标成本(含税)(元)">
          <template slot-scope="scope">
            <span v-if="scope.row.expenseItem == '【间接费】' || scope.row.expenseItem == '【材料费】' || scope.row.expenseItem == '【人工费】'">
              {{scope.row.targetAmountTax || msgTip.AUTO_PLACEHOLDER}}
            </span>
            <amount-input v-else v-model="scope.row.targetAmountTax" size="small" placeholder="请输入" @change="changeTotalTargetAmountUntax($event,scope.row)"/>
          </template>
        </el-table-column>
        <el-table-column label="目标成本(无税)(元)">
          <template slot-scope="scope">
            <span v-if="scope.row.expenseItem == '【间接费】' || scope.row.expenseItem == '【材料费】' || scope.row.expenseItem == '【人工费】'">
              {{scope.row.targetAmountUntax || msgTip.AUTO_PLACEHOLDER}}
            </span>
            <amount-input v-else v-model="scope.row.targetAmountUntax" size="small" placeholder="请输入" @change="changeTotalTargetAmountUntax($event,scope.row)"/>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-table :key="tableKey2" :data="dataForm.planCostCompareDetailList" class="f2bpmui-datagrid-vuetable" border width="100%">
        ...
        <el-table-column label="投标报价(按甲方清单报价格式)" align="center">
          ...
          <el-table-column label="税率" width="160px">
            ...
            <template slot-scope="scope">
              <select-dialog2  :disabled="true"  :modelData="rateModel" :width="900" :btnDisabled="setting.readonly"
      ...
      <row-layout :cols="2">
        <el-form-item slot="col_1" label="投标报价税金(元)：" prop="bidPriceTaxation">
          <amount-input v-model="dataForm.bidPriceTaxation" defaultValue clearable
                        :placeholder="msgTip.ENTER_PLACEHOLDER"></amount-input>
        </el-form-item>
        <el-form-item slot="col_2" prop="customerPhone" label="计划成本税金(元)：">
          <amount-input v-model="dataForm.planCostTaxation" @change="planCostTaxationChange" defaultValue clearable
                        :placeholder="msgTip.ENTER_PLACEHOLDER"></amount-input>
        </el-form-item>
      </row-layout>
      <el-table :data="dataForm.planCostTaxationDetailList" class="f2bpmui-datagrid-vuetable" border>
        <el-table-column label="No." width="100px">
          <template slot-scope="scope">
        ...
        <el-table-column label="计划成本(元)">
          <template slot-scope="scope">
            <amount-input v-model="scope.row.planAmount" size="small" placeholder="请输入"/>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  import {getConstructInfo,checkOnlyData,getLastestInfo} from '@/Api/planCostMain/index.js'
  ...
  export default {
    ...
    data() {
      return {
        ...
        dataForm: {
          ...
          fileInfoList: [],
          id: '',
          planCostCompareDetailList: [], // 计划成本材料对比表
          planCostFeeDetailList: [], // 计划成本料工费明细
          planCostSummaryDetailList: [],  // 计划成本汇总表
          planCostTaxation: '',
          planCostTaxationDetailList: [], // 计划成本税金
          procInstState: '',
          projectAddress: '',
          projectManagerName: '',
          remark: '',
          ...
          singleTargetAmount: '',
          totalContructionAmountTax: '',
          totalDesignAmountTax: '',
          totalSaleAmountTax: '',
          wiid: '',
          constructionUnitId:'',
          projectManagerId:''
        ...
        //添加必填项 字段
        //必填项字段
        dataRule: RuleRequired.dataRule(['marketUnitName','designTaxRate','saleTaxRate','constructionTaxRate','buildArea','bidPriceTaxation','constructionAmountTax','fileInfoList' ])
      }
    },
    computed: {
    },
    methods: {
      ...
      beforeInit() {
        ...
        // 汇总表
        this.dataForm.planCostSummaryDetailList = this.getSummeryData;
        // 料工费明细
        this.dataForm.planCostFeeDetailList = this.getFeeData;
        // 税金
        this.dataForm.planCostTaxationDetailList = this.getCostData;
      },
      //获取详情后的赋值情况
      afterInit() {
        var that = this;
        console.log(this.dataForm, '2222222222222222');
        const list = this.dataForm.planCostCompareDetailList;
        if(list && list.length > 0){
          that.importPlanCostCompareCount(list,0)
        }
        that.dataForm.totalContructionAmountTax = this.dataForm.totalConstructionAmountUntax;
        that.dataForm.totalDesignAmountTax = this.dataForm.totalDesignAmountUntax;
        that.dataForm.totalSaleAmountTax = this.dataForm.totalSaleAmountUntax;
        if(that.dataForm.id){
          that.getLastestFn();
        }
      },
      ...
      async getConstructInfoFn(){
        ...
        if(res.code === 200){
          ...
          this.dataForm.accumulateImageProgressRate = data.accumulateImageProgressRate;
          this.dataForm.constructionAmountTax = data.constructionAmountTax;
          this.dataForm.changeConstructionAmountTax = data.changeConstructionAmountTax;
          this.dataForm.designAmountTax = data.designAmountTax;
          this.dataForm.changeDesignAmountTax = data.changeDesignAmountTax;
          this.dataForm.saleAmountTax = data.saleAmountTax;
          this.dataForm.changeSaleAmountTax = data.changeSaleAmountTax;
          this.dataForm.constructionTaxRate = data.constructionTaxRate;
          this.dataForm.designTaxRate = data.designTaxRate;
          this.dataForm.saleTaxRate = data.saleTaxRate;
          this.dataForm.constructionUnitId = data.constructionUnitId;
          this.dataForm.projectManagerId = data.projectManagerId;
          const list = data.planCostSummaryDetailList;
          const list2 = this.dataForm.planCostSummaryDetailList; // 汇总表
          if(list.length > 0){
            for(let i in list){
              for(let j in list2){
                if(list[i].totalExpenseItem === list2[j].totalExpenseItem){
                  const {contractOutAmount, contractOutAmountRate, payOutAmount,payOutAmountRate} = list[i];
                  list2[j].contractOutAmount = contractOutAmount;
                  list2[j].contractOutAmountRate = contractOutAmountRate;
                  list2[j].payOutAmount = payOutAmount;
                  list2[j].payOutAmountRate = payOutAmountRate;
                }
              }
            }
          }
      ...
      // 新增--材料对比表
      handelAdd(){
        if(!this.dataForm.planCostCompareDetailList) this.dataForm.planCostCompareDetailList = [];
        this.dataForm.planCostCompareDetailList.push({
          bidAmountTax: '',
          bidClearPoint: '',
          bidClearRatio: '',
          bidPriceTax: '',
      ...
      changeTotalTargetAmountUntax(e,row){
        var amount_1 = 0;
        var amount_2 = 0;
        var list = this.dataForm.planCostFeeDetailList; //  料工费明细
        var plannedCostSummary = this.dataForm.planCostSummaryDetailList; // 汇总表
        ...
        for(let i in arr){
          amount_1+=Number(arr[i]['targetAmountTax']);
          amount_2+=Number(arr[i]['targetAmountUntax']);
        }
        amount_1 = Object.tofixed(amount_1,2);
        amount_2 = Object.tofixed(amount_2,2);
        for(let j in list){
          if(type == 1 && list[j].expenseItem == '【人工费】'){
            this.$set(this.dataForm.planCostFeeDetailList[j], 'targetAmountTax', amount_1);
            this.$set(this.dataForm.planCostFeeDetailList[j], 'targetAmountUntax', amount_2);
            plannedCostSummary.filter(item => item['totalExpenseItem'] === '人工费')[0]['totalTargetAmountTax'] = amount_1;
            plannedCostSummary.filter(item => item['totalExpenseItem'] === '人工费')[0]['totalTargetAmountUntax'] = amount_2;
          }else if(type == 2 && list[j].expenseItem == '【材料费】'){
            this.$set(this.dataForm.planCostFeeDetailList[j], 'targetAmountTax', amount_1);
            this.$set(this.dataForm.planCostFeeDetailList[j], 'targetAmountUntax', amount_2);
            plannedCostSummary.filter(item => item['totalExpenseItem'] === '材料费')[0]['totalTargetAmountTax'] = amount_1;
            plannedCostSummary.filter(item => item['totalExpenseItem'] === '材料费')[0]['totalTargetAmountUntax'] = amount_2;
          }else if(type == 3 && list[j].expenseItem == '【间接费】'){
            this.$set(this.dataForm.planCostFeeDetailList[j], 'targetAmountTax', amount_1);
            this.$set(this.dataForm.planCostFeeDetailList[j], 'targetAmountUntax', amount_2);
            plannedCostSummary.filter(item => item['totalExpenseItem'] === '间接费')[0]['totalTargetAmountTax'] = amount_1;
            plannedCostSummary.filter(item => item['totalExpenseItem'] === '间接费')[0]['totalTargetAmountUntax'] = amount_2;
          }
        }
      ...
      // 联动计算 -- 汇总表
      calculateByPlanCost(){
        var plannedCostSummary = this.dataForm.planCostSummaryDetailList;
        ...
        let sumAmount0 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '人工费')[0]['totalTargetAmountTax'];
        let sumAmount1 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '材料费')[0]['totalTargetAmountTax'];
        let sumAmount2 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '专业分包合同')[0]['totalTargetAmountTax'];
        let sumAmount3 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '间接费')[0]['totalTargetAmountTax'];
        let sumAmount4 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '利息支出(资金占用费)')[0]['totalTargetAmountTax'];
        let sumAmount00 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '人工费')[0]['totalTargetAmountUntax'];
        let sumAmount11 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '材料费')[0]['totalTargetAmountUntax'];
        let sumAmount22 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '专业分包合同')[0]['totalTargetAmountUntax'];
        let sumAmount33 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '间接费')[0]['totalTargetAmountUntax'];
        let sumAmount44 = plannedCostSummary.filter(item => item['totalExpenseItem'] === '利息支出(资金占用费)')[0]['totalTargetAmountUntax'];
        let sumAmount5 = this.dataForm.planCostTaxation; // 增值税附加
        ...
        allSumAmount1 = Object.tofixed(allSumAmount1,2);
        if(allSumAmount1){
          plannedCostSummary.filter(item => item['totalExpenseItem'] === '项目成本合计')[0]['totalTargetAmountTax'] = Number(allSumAmount1) ;
        }
        ...
        allSumAmount2 = Object.tofixed(allSumAmount2,2);
        if(allSumAmount2){
          plannedCostSummary.filter(item => item['totalExpenseItem'] === '项目成本合计')[0]['totalTargetAmountUntax'] = Number(allSumAmount2);
        }
        let val3 = 0;
        if (Number(amountTax) === 0) {
          ...
          val3 = Object.tofixed(val3,4);
          plannedCostSummary.filter(item => item['totalExpenseItem'] === '项目利润率(%)')[0]['totalTargetAmountTax'] = Number(val3);
        }
        let val33 = 0;
        if (Number(amountNoTax) === 0) {
          ...
          val33 = Object.tofixed(val33,4);
          plannedCostSummary.filter(item => item['totalExpenseItem'] === '项目利润率(%)')[0]['totalTargetAmountUntax'] =  Number(val33);
        }
        ...
        sumTotalTargetAmountTax = plannedCostSummary.filter(item => item['totalExpenseItem'] === '项目成本合计')[0]['totalTargetAmountTax'];
      ...
      sumAmountTaxChange(e,name){
        ...
        if(name === 'constructionAmountTax'){
          let contructionAmountTax = that.dataForm.changeContructionAmountTax; // --施工金额增补(含税)
        ...
        var currentContractAmountUntax = Number(that.dataForm.totalContructionAmountTax) + Number(that.dataForm.totalDesignAmountTax) + Number(that.dataForm.totalSaleAmountTax);
        that.dataForm.currentContractAmountUntax = Object.tofixed(currentContractAmountUntax,2); // 现合同金额(无税)
        var currentContractAmountTax = Number(that.dataForm.constructionAmountTax) + Number(that.dataForm.changeContructionAmountTax) + Number(that.dataForm.designAmountTax)+Number(that.dataForm.changeDesignAmountTax)+Number(that.dataForm.saleAmountTax)+Number(that.dataForm.changeSaleAmountTax);
      ...
      bidRateRateUpdate(data,index){
        const {taxRate, id} = data[0];
        this.$set(this.dataForm.planCostCompareDetailList[index], 'bidRate', taxRate);
      },
      // 对比表 --目标成本税率
      targetRateUpdate(data,index){
        const {taxRate, id} = data[0];
        this.$set(this.dataForm.planCostCompareDetailList[index], 'targetRate', taxRate);
      },
      // 保留两位小数
      ...
      changeTofixed4(e,index,name){
        var that = this;
        var val = (e).toString()?Number(e):'';
        if(typeof val == 'number'){
          val = Object.tofixed(val,4);
          that.$set(that.dataForm.planCostCompareDetailList[index], name, val);
        }else {
          that.$set(that.dataForm.planCostCompareDetailList[index], name, '');
        }
        if(name == 'projectQuantity' || name == 'bidPriceTax'){
          var quantity = that.dataForm.planCostCompareDetailList[index]['projectQuantity']; //工程量
          var priceTax = that.dataForm.planCostCompareDetailList[index]['bidPriceTax']; //投标 单价（含税）
          if(quantity && priceTax){
            let amount1 = Number(quantity) * Number(priceTax);
            that.$set(that.dataForm.planCostCompareDetailList[index], 'bidAmountTax', Object.tofixed(amount1,2)); //投标 金额
          }
        }else if(name == 'projectQuantity' || name == 'targetPriceTax'){
          var quantity3 = that.dataForm.planCostCompareDetailList[index]['projectQuantity']; //工程量
          var targetPriceTax = that.dataForm.planCostCompareDetailList[index]['targetPriceTax']; //目标成本 单价（含税）
          if(quantity3 && targetPriceTax){
            let amount3 = Number(quantity3) * Number(targetPriceTax);
            that.$set(that.dataForm.planCostCompareDetailList[index], 'targetAmountTax', Object.tofixed(amount3,2)); //目标成本 金额
          }
        }else if(name == 'bidPriceUntax' || name == 'targetPriceUntax'){
          var bidPriceUn = that.dataForm.planCostCompareDetailList[index]['bidPriceUntax']; //投标单价（无税）
          var targetPriceUn = that.dataForm.planCostCompareDetailList[index]['targetPriceUntax']; //目标成本单价（无税）
          ...
          that.$set(that.dataForm.planCostCompareDetailList[index], 'differencePrice', Object.tofixed(differencePrice,2)); //价差
          var targetRatio = '';
          if(bidPriceUn != 0){
             targetRatio = Number(differencePrice)/Number(bidPriceUn); //价差/投标报价的 单价(无税)
             targetRatio = Object.tofixed(targetRatio,4)
          }
          that.$set(that.dataForm.planCostCompareDetailList[index], 'targetRatio', targetRatio); //目标成本 比例
          var targetPoint = '';
          ...
          that.$set(that.dataForm.planCostCompareDetailList[index], 'targetPoint', targetPoint); //目标成本 经营点
        } else if(name == 'pureQuantity'){
          // 清标工程量
          var pureQuantity = that.dataForm.planCostCompareDetailList[index]['pureQuantity']; //净用量
          var wasteRate = that.dataForm.planCostCompareDetailList[index]['wasteRate']; // 损耗率
          var quantity2 = that.dataForm.planCostCompareDetailList[index]['projectQuantity']; //工程量
          if(pureQuantity && wasteRate){
            let amount2 = Number(pureQuantity) * (1+Number(wasteRate));
            amount2 = Object.tofixed(amount2,2);
            that.$set(that.dataForm.planCostCompareDetailList[index], 'planQuantity', amount2); // 计划用量
            var diffQuantity = Number(amount2)-Number(quantity2) ; // 计划用量-工程量
            diffQuantity = Object.tofixed(diffQuantity,2);
            that.$set(that.dataForm.planCostCompareDetailList[index], 'differenceQuantity', diffQuantity); // 量差
            if(quantity2 != 0){
              var bidClearRatio = Number(diffQuantity)/Number(quantity2); // 量差/工程量
              bidClearRatio = Object.tofixed(bidClearRatio,4);
              that.$set(that.dataForm.planCostCompareDetailList[index], 'bidClearRatio', bidClearRatio); // 比例
              ...
              that.$set(that.dataForm.planCostCompareDetailList[index], 'bidClearPoint', bidClearPoint); //清标工程量 经营点
            }
      ...
      changeWasteRate(e,index){
        ...
        if(pureQuantity && wasteRate){
          ...
          that.$set(that.dataForm.planCostCompareDetailList[index], 'bidClearPoint', bidClearPoint); //目标成本 经营点
        }
      },
      // 联动计算 -- 建筑面积
      calculateByBuildArea(){
        ...
        const plannedCostSummary = this.dataForm.planCostSummaryDetailList;
        let sumTotalTargetAmountTax = 0; // 汇总表tab的目标成本(含税)所在列的"项目成本合计"
        sumTotalTargetAmountTax = plannedCostSummary.filter(item => item['totalExpenseItem'] === '项目成本合计')[0]['totalTargetAmountTax'];
        if(num && sumTotalTargetAmountTax){
          let amount2 = sumTotalTargetAmountTax/num;
      ...
      planCostTaxationChange(){
        var plannedCostSummary = this.dataForm.planCostSummaryDetailList;
        var val = this.dataForm.planCostTaxation;
        plannedCostSummary.filter(item => item['totalExpenseItem'] === '增值税及附加')[0]['totalTargetAmountTax'] = Number(val);
        this.calculateByPlanCost();
        this.$forceUpdate();
      },
      // 汇总表--项目分包合同 or 利息支出
      ...
      async vaildateOnlyData(){
        ...
        let res = await checkOnlyData({constructArchiveMainId: id});
        if(res.code === 200){
          return true;
        }else{
      ...
      validateForm() {
        const _this = this;
        var list = _this.dataForm.planCostCompareDetailList;
        if(list.length === 0){
      ...
      vaildTableList(){
        var isOk = false;
        // 判断table内输入框必填
        var arr = this.dataForm.planCostCompareDetailList; // 对比表
        if (arr.length > 0) {
      ...
      async handelExportSummary(){
        let isOk = false;
        let url = '/construct/planCostSummaryDetail/exportData';
      ...
      // 料工费明细导入
       handelImportPlanCostFee(){
        ...
        this.commonImportData.tableName = 'planCostFeeDetailList';
        this.keyWord = 'planCostFeeDetail';
        this.modelName = 'construct'
      },
      // 料工费明细导出
      async handelExportPlanCostFee(){
        let isOk = false;
        let url = '/construct/planCostFeeDetail/exportData';
        let id = this.dataForm.id;
        if(id){
          let param = {
            constructPlanCostMainId: id
      ...
      async handelExportPlanCostCompare(){
        let isOk = false;
        let url = '/construct/planCostCompareDetail/exportData';
        let id = this.dataForm.id;
        if(id){
          let param = {
            constructPlanCostMainId: id
      ...
      handelImportPlanCostCompare(){
        this.commonImportData.visible = true;
        this.commonImportData.tableName = 'planCostCompareDetailList';
        this.keyWord = 'planCostCompareDetail';
        this.modelName = 'construct'
      },
      // 导入返回
      importSuccess(tableName,rows){
        if(rows && rows.length >0){
          if(tableName === 'planCostFeeDetailList'){
            console.log(rows,'123');
            const list = this.dataForm.planCostFeeDetailList;
            for(let i in rows){
              for(let j in list){
          ...
          }else if(tableName === 'planCostCompareDetailList'){
      ...
      async getLastestFn(){
        let id = this.dataForm.constructArchiveMainId;
        let res = await getLastestInfo({constructArchiveMainId: id});
        if(res.code === 200){
          const {data} = res;
          this.planCostLink = data?data:'';
        }
      },
    }
  };
