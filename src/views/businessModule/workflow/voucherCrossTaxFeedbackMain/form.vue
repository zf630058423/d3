<template>
  ...
  <div class="bank-files-form">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="160px" :disabled="setting.readonly">
      ...
      <row-layout :cols="3" title="反馈信息" marginStyle="margin: 16px 16px 0 16px">
        <template slot="col_1">
          ...
          <el-form-item prop="feedbackDate" label="反馈日期：">
            <el-date-picker v-model="dataForm.feedbackDate" value-format="yyyy-MM-dd" type="date"
        ...
        <template slot="col_2">
          ...
          <el-form-item prop="feedBackPersonName" label="反馈登记人：">
            <select-dialog :width="900" :modelData='{parm:{singleSelect: true}}'
                         :height="360" @updateValue="feedBackPersonNameUpdate"
                         :value="dataForm.feedBackPersonName"
...
export default {
  name: "voucherCrossTaxFeedbackMain",
  components: {
    RowLayout,
    updateFiles,
    SelectDialog,
  ...
  data () {
    return {
      dataForm: {
        ...
        deptId: "", //编制人部门
        deptName: "", //编制人部门
        feedBackPersonName: "", //反馈登记人
        feedBackPersonId: "", //反馈登记人id
      },
      autofillPlaceHolder: "自动带出", //自动填写
      pleaseEnterPlaceHolder: "请输入", //请输入
      radioOptions: RADIO3_GROUP,
      voucherCrossModelData: { code: 'KuaShuiBaoHaoTongYong', parm: { varParams: [{ key: 'crossReportState', value: '6' }, { key: 'constructArchiveMainId', value: '' }] } }, //跨税报号（通用）已报验
      ...
      dataRule: RuleRequired.dataRule(['contractCode', 'constructCode', 'contactPersonRrossTaxRelated',
        'contactInformation', 'isNullifyReopen', 'taxUnitName', 'stampTaxMethodPayment', 'fileInfoList','taxAcross','feedBackPersonName','feedbackDate']),
  ...
  created () {
    const { userId, realName } = this.userInfo
    if(WFContext.WorkflowInstinceState == 0){
      this.dataForm.feedBackPersonName = realName;
      this.dataForm.feedBackPersonId = userId;
    }
  ...
  methods: {
   ...
   voucherCrossMainCode (data) {
    ...
    // 反馈登记人
    feedBackPersonNameUpdate(data){
      this.dataForm.feedBackPersonName = data[0].realName;
      this.dataForm.feedBackPersonId = data[0].userId;
    },
    async validateForm () {
      const _this = this;
