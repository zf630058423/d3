    ...
    >
      <row-layout title="业务单元信息" :cols="3">
        <el-form-item slot="col_1" prop="marketUnitName" label="业务单元：">
          <select-dialog :disabled="true" :btnDisabled="setting.readonly" :width="800" :modelData="marketModelData" :height="360" :value="dataForm.marketUnitName" @updateValue="marketUnitUpdateValue" dynamicModel="customDialog"></select-dialog>
        </el-form-item>
          <el-form-item slot="col_2" prop="marketCenterName" label="业务中心：">
            <el-input v-model="dataForm.marketCenterName" disabled :placeholder="msgTip.AUTO_PLACEHOLDER" />
          </el-form-item>
          <el-form-item slot="col_3" prop="followerName" label="跟进人：">
            <select-dialog :disabled="true" :btnDisabled="setting.readonly" :width="900" :height="360" :modelData="dialogModel" @updateValue="followerUpdate" :value="dataForm.followerName || msgTip.SELECT_PLACEHOLDER" dynamicModel="selectUsers"></select-dialog>
        </el-form-item>
      </row-layout>
      <row-layout title="立项信息" :cols="3">
        <el-form-item slot="col_1" prop="name" label="立项名称：">
            <text-input v-model="dataForm.name" :placeholder="msgTip.ENTER_PLACEHOLDER" />
        </el-form-item>
        <el-form-item slot="col_2" prop="code" label="立项编号：">
            <el-input v-model="dataForm.code" disabled :placeholder="msgTip.AUTO_PLACEHOLDER" />
        </el-form-item>
        <el-form-item slot="col_3" prop="productLineName" label="产品线：">
            <select-dialog :disabled="true" :btnDisabled="setting.readonly" :width="900" :height="360" :modelData="productLineModel" @updateValue="productLineUpdate" :value="dataForm.productLineName || msgTip.SELECT_PLACEHOLDER" dynamicModel="customDialog"></select-dialog>
        </el-form-item>
      </row-layout>
      <row-layout :offset="0" :cols="3">
        <el-form-item slot="col_1" prop="projectAmount" label="工程造价(元)：">
            <amount-input v-model="dataForm.projectAmount" @change="projectAmountChagne" defaultValue clearable :placeholder="msgTip.ENTER_PLACEHOLDER"></amount-input>
          </el-form-item>
        <el-form-item slot="col_2" prop="projectLeaderName" label="项目介绍人：">
            <select-dialog :disabled="true" :btnDisabled="setting.readonly" :width="950" :height="360" :modelData="projectLeaderModel" @updateValue="projectleaderNameUpdate" :value="dataForm.projectLeaderName || msgTip.SELECT_PLACEHOLDER" dynamicModel="customDialog"></select-dialog>
        </el-form-item>
        <el-form-item slot="col_3" prop="projectType" label="立项类型：">
            <el-select v-model="dataForm.projectType" :placeholder="msgTip.SELECT_PLACEHOLDER" @change="marketTypeChange">
            <el-option
      ...
      <row-layout :cols="3">
        <el-form-item slot="col_1" prop="customerName" label="客户：">
          <select-dialog :disabled="true" :btnDisabled="setting.readonly" :width="900" :height="360" :modelData="customerModel" @updateValue="customerUpdate" :value="dataForm.customerName || msgTip.SELECT_PLACEHOLDER" dynamicModel="customDialog"></select-dialog>
        </el-form-item>
        <el-form-item slot="col_2" prop="qualificationScopeName" label="资质范围：">  
          <select-dialog :disabled="true" :btnDisabled="setting.readonly" :width="900" :height="360" :modelData="qualificationScopeModel" @updateValue="qualificationScopeUpdate" :value="dataForm.qualificationScopeName || msgTip.SELECT_PLACEHOLDER" dynamicModel="customDialog"></select-dialog>
        </el-form-item>
        <el-form-item slot="col_3" prop="marketCategory" label="立项类别：">
            <el-select v-model="dataForm.marketCategory" @change="marketCategoryChange" :placeholder="msgTip.SELECT_PLACEHOLDER">
            <el-option
      ...
      <row-layout :cols="3">
        <el-form-item slot="col_1" prop="customerContact" label="客户联系人：">
          <el-input v-model="dataForm.customerContact" :placeholder="msgTip.ENTER_PLACEHOLDER" />
        </el-form-item>
        <el-form-item slot="col_2" prop="customerPhone" label="客户联系电话：">
            <el-input v-model="dataForm.customerPhone" :placeholder="msgTip.ENTER_PLACEHOLDER" />
        </el-form-item>
        <el-form-item slot="col_3" prop="marketStatus" label="项目当前状态：">
            <el-select v-model="dataForm.marketStatus" @change="marketStatusChange" disabled :placeholder="msgTip.SELECT_PLACEHOLDER">
              <el-option
      ...
      <row-layout :cols="3" cell="1-2">
        <el-form-item slot="col_1" prop="address" label="项目地址（省-市）：">
            <casc-select code="AddressCascade" :defaultValue="dataForm.address" @change-select="changeAdressSelect"></casc-select>
          </el-form-item>
         <el-form-item slot="col_2" prop="detailAddress" label="详细地址：">
            <el-input v-model="dataForm.detailAddress" :placeholder="msgTip.ENTER_PLACEHOLDER" />
          </el-form-item>
      </row-layout>
      <row-layout :cols="3" cell="3">
        <el-form-item slot="col_1" prop="remark" label="备注：">
            <el-input type="textarea" :placeholder="msgTip.ENTER_PLACEHOLDER" rows="5" v-model="dataForm.remark"></el-input>
          </el-form-item>
      </row-layout>
      <row-layout title="招标内容及要求" :cols="3">
        <el-form-item slot="col_1" prop="estimateBidDate" label="预估投标日期：">
          <el-date-picker v-model="dataForm.estimateBidDate" type="date" placeholder="选择日期"></el-date-picker>
        </el-form-item>
        <el-form-item slot="col_2" prop="bidForm" label="招投标形式：">
           <el-select v-model="dataForm.bidForm" @change="bidFormChange" :placeholder="msgTip.SELECT_PLACEHOLDER">
            <el-option
        ...
        <el-form-item slot="col_3" prop="isPublicInternet" label="网上公示：">
          <el-radio-group v-model="dataForm.isPublicInternet">
      ...
      <row-layout :cols="3">
         <el-form-item slot="col_1" prop="isLocalFilingRequired" label="是否要求当地备案：">
           <el-radio-group v-model="dataForm.isLocalFilingRequired">
              <el-radio :label="item.label" :key="item.label" v-for="item in radioData">{{ item.name }}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item slot="col_2" prop="isLock" label="是否锁证：">
          <el-radio-group v-model="dataForm.isLock" @change="isLockChange">
            <el-radio :label="item.label" :key="item.label" v-for="item in radioData">{{ item.name }}</el-radio>
          </el-radio-group>
        </el-form-item>
      ...
      <row-layout :cols="3">
         <el-form-item slot="col_1" prop="isLocalConstructionRequired" label="是否要求当地报建：">
          <el-radio-group v-model="dataForm.isLocalConstructionRequired">
            <el-radio :label="item.label" :key="item.label" v-for="item in radioData">{{ item.name }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item slot="col_2" prop="constructorRequirement" label="建造师要求：">
           <el-select v-model="dataForm.constructorRequirement" @change="constructorChange" :placeholder="msgTip.SELECT_PLACEHOLDER">
            <el-option
        ...
        <el-form-item slot="col_3" prop="constructorAbnormal" label="建造师要求是否异常：">
          <el-radio-group v-model="dataForm.constructorAbnormal">
      ...
      <row-layout title="附件" :cols="3" cell="3">
         <el-form-item slot="col_1" prop="fileInfo" label="附件：">
          <uploader-file v-model="dataForm.fileInfo" :revise="isReadOnlyStatus" :isReadOnly="isReadOnlyStatus"></uploader-file>
        </el-form-item>
      </row-layout>
...
export default {
  components: { FormTitle, RowLayout, SelectDialog, AmountInput,TextInput, CascSelect, UploaderFile },
  mixins: [comMixin],
  data() {
    return {
      ...
      projectIntroducerStatus: '',  //项目介绍人预警状态
      ...
      //添加必填项 字段
      //必填项字段
      dataRule: RuleRequired.dataRule(['marketUnitName', 'followerName','itemName','name', 'productLineName', 'projectAmount', 'marketType','lockTime','customerName', 'qualificationScopeName', 'projectType', 'marketCategory', 'customerContact',
  ...
  methods: {
    ...
    //获取详情后的赋值情况 8274be77-3050-47dc-a271-046cdcbcd945
    afterInit() { 
      ...
      if (!this.dataForm.closeState) { 
        this.dataForm.closeState = 1;
      }
      this.checkProjectManager();
      // this.initData();
    },
    initData() { 
      //是否要求当地备案:isLocalFilingRequired  网上公示：isPublicInternet  是否锁证：isLock 是否要求当地报建： isLocalConstructionRequired
      let { isLocalFilingRequired,isPublicInternet, isLock, isLocalConstructionRequired, constructorAbnormal } = this.dataForm;
    ...
    marketTypeChange(e) {
      ...
      this.qualificationScopeModel.parm.varParams.find(item => item.key === 'project_type').value = e || '';
    },
    //招投标形式
    bidFormChange(e) {
    ...
    //项目介绍人  projectIntroducerStatus 1:正常 2： 关注  3：预警
    projectleaderNameUpdate(data) { 
      console.log("项目介绍人===：", data);
    ...
    marketUnitUpdateValue(data) {
     if (data && data.length > 0) { 
        this.dataForm.marketUnitId = data[0].id;  //业务单元id  marketUnitId
    ...
    async getDataDictByConfigType(configTypes) { 
      ...
      if (success) { 
        if (data && data.length > 0) { 
          ...
          this.bidFormArray = this.bidFormArray.filter(el=>{
            return el.configName!=="预选承包商"; // 去掉招投标形式里面的预选承包商
          });
        ...
        console.log("项目当前状态===：", this.marketStatusArray);
      }
    },
    updateValue(data) { 
   ...
   async validateForm() {
     const _this = this;
      //提交时，  预警阶段 且 立项为 施工、设计施工一体化时  校验 不可提交  cpmStatus(false:预警)
     ...
     if (!_this.cpmStatus) {
      ...
      //   return false;
      // } 
      //审批中 projectIntroducerStatus 1:正常 2： 关注  3：预警
    ...
    constructorAbnormalJudge() {
      ...
      if (bidForm == 1 || bidForm == 2) { //招标投标为政府公开或其他公开
        if (Number(projectAmount) < num) { //工程造价小于1000万
          ...
          this.dataForm.constructorAbnormal = (constructorRequirement == '一级') ? '1' : '0';
        } else {
