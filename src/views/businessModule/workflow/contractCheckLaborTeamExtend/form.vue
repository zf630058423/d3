  ...
  <div v-loading="loading">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" :disabled="setting.readonly" label-width="140px" class="new-data-form-css">
      ...
      <row-layout :cols="3" :title="'合同信息'">
        ...
        <el-form-item slot="col_3" prop="teamLeader" label="班组长：">
          <selector-dialog :model-data="teamLeaderDialog" :value="dataForm.teamLeader" dynamicModel="customDialog" @updateValue="updateTeamLeader"></selector-dialog>
        </el-form-item>
      </row-layout>
      <row-layout :cols="3">
        <el-form-item slot="col_1" prop="thisNuclearQuantityAmount" label="本次核量金额(元)：">
          <amount-input v-model="dataForm.thisNuclearQuantityAmount" disabled size="small" placeholder="自动计算" />
        </el-form-item>
        <el-form-item slot="col_2" prop="approvedAmount" label="已核量金额(元)(不含本次)：">
      ...
      <row-layout :cols="3">
        <el-form-item slot="col_1" prop="cumulativeSupervisionAmount" label="甲方/监理累计确认：">
          <amount-input v-model="dataForm.cumulativeSupervisionAmount" size="small" placeholder="自动计算" disabled/>
        </el-form-item>
        <el-form-item slot="col_2" label="本次核量日期区间：" required>
          <el-col :span="11" style="padding-right: 0;padding-left: 0">
            <el-form-item prop="thisNuclearStartTime">
              <el-date-picker v-model="dataForm.thisNuclearStartTime" @blur="vaildTime" type="date" placeholder="开始时间" size="small" value-format="yyyy-MM-dd">
              </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="2" style="text-align: center">-</el-col>
          <el-col :span="11"  style="padding-right: 0;padding-left: 0">
            <el-form-item prop="thisNuclearEndTime">
              <el-date-picker v-model="dataForm.thisNuclearEndTime" @blur="vaildTime" type="date" placeholder="结束时间" size="small" value-format="yyyy-MM-dd">
              </el-date-picker>
            </el-form-item>
          </el-col>
        </el-form-item>
      ...
      <row-layout :cols="3">
        <el-form-item slot="col_1" prop="teamPeople" label="班组人数：">
          <numberInput v-model="dataForm.teamPeople" size="small" placeholder="请输入" />
        </el-form-item>
        <el-form-item slot="col_3" label="人工总考勤天数：" prop="totalAttendanceDay">
          <el-input v-model="dataForm.totalAttendanceDay" @change="changeTofixed($event,'totalAttendanceDay')" size="small" placeholder="请输入" />
        </el-form-item>
      </row-layout>
      <row-layout :cols="1">
  ...
  export default {
    ...
    data() {
      return {
        ...
        radioOptions: [],
        teamLeaderDialog: {
          code: 'BanZuKu', customDialog: 'BanZuKu', isSetHead: 1, isMulti: false, initailData: []
        }, // 班组长 BanZuKu
    ...
    methods: {
      ...
      updateMainContractValue(data) {
        if(data && data.length <= 0) return;
        const { mastercontractid, mastercontractcode, mastercontractname,teamleader,teamlibraryid} = data[0];
      ...
      async getContractAmountInfoFn(){
        ...
        let res = await getContractAmountInfo(params);
        if(res.code === 200){
          const {approvedAmount,cumulativeSupervisionAmount,totalContractAmount} = res.data;
          this.dataForm.approvedAmount = approvedAmount; // 已核量金额(元)(不含本次)
          this.dataForm.cumulativeSupervisionAmount = cumulativeSupervisionAmount; //甲方监理累计确认
