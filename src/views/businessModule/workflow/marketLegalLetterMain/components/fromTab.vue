  ...
  <div class="accountInfoClass">
    <el-form ref="subFrom" :model="subFormData" :rules="rules" :disabled="disabled">
      ...
      <el-table :data="subFormData.detailList" style="width: 100%" size="small" fit class="f2bpmui-datagrid-vuetable"
        ...
        <el-table-column label="被委托人" :class="'required'">
...
export default {
  ...
  methods: {
    ...
    handleAdd () {
      ...
      const obj = {
        age: "",
        marketLegalLetterMainId: rowId || "", //法人授权委托书主键
        identityCard: "", //身份证号
        name: "", //姓名
        sex: "", //性别
      };
    ...
    validateForm () {
      ...
      for (let index = 0; index < _keepData.length; index++) {
        ...
        for (let key in validObj) {
          if (!_row[key]) {
            this.$message.error(`被委托人信息第 ${_currentPage} 页，第 ${(parseInt(index) + 1)%_pageSize} 行 ${validObj[key]} 不能为空`)
            return false
          }
        }
      }
