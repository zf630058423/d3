<template>
  <div class="supplierMaterialExtend-register">
    <div class="header">
      <div class="btn-box">
    ...
    <div class="form-box" v-loading="loading">
      ...
      >
        ...
        <row-layout :cols="3">
          ...
          <el-form ref="warehouseForm" :model="dataForm" :rules="dataRule" :disabled="false" slot="col_2" label-width="180px">
...
<script>
import {checkSupplierLinkSecretKey,supplierLinkSave} from '@/Api/workflowBusssion/supplier.js'
...
export default {
  ...
  methods: {
    ...
    async initData() {
      ...
      const res = await checkSupplierLinkSecretKey(code)
      if(res.code == 200){
        this.dataForm.createUserId = res.data.createId;
        this.dataForm.createUserName = decodeURIComponent(res.data.createName);
        this.dataForm.deptId = res.data.createDeptId;
        this.dataForm.deptName = decodeURIComponent(res.data.createDept);
    ...
    async submitForm() {
      const _this = this;
      const validateTable = await _this.validateTable()
      if(!validateTable) return false;
      _this.$refs['dataForm'].validate(async valid => {
        if (valid) {
          const res = await supplierLinkSave(_this.getDataForm())
          if(res.code == 200){
...
.supplierMaterialExtend-register {
  width: 100%;
  background: #f7fafc;
  padding-top: 70px;
  padding-bottom: 20px;
