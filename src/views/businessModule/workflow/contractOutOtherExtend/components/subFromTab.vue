  ...
  <div class="accountInfoClass">
    <el-form ref="subFrom" :model="subFormData" :rules="rules" :disabled="disabled">
      ...
      <el-table :data="subFormData.detailList" style="width: 100%" size="small" fit class="f2bpmui-datagrid-vuetable"
        ...
        <el-table-column label="税率" :align="'center'" width="180">
          <template slot-scope="scope">
        ...
        <el-table-column label="税额" :align="'center'">
          <template slot-scope="scope">
            <el-input type="text" v-model="scope.row.taxAmount" size="small" disabled placeholder="自动计算" />
          </template>
        </el-table-column>
        <el-table-column label="金额（无税）" :align="'center'">
          <template slot-scope="scope">
            <el-input type="text" v-model="scope.row.amountUntax" @change="updateContractAmount" size="small" disabled placeholder="自动计算" />
          </template>
        </el-table-column>
...
export default {
  ...
  data () {
    return {
      ...
      importData: {
        visible: false,
        url: '/contract/contractOutOtherExtend/importDetailData',
        key: 'contractOutOtherDetail'
      },
  ...
  methods: {
    ...
    updateTaxRateValue (index, data) {
      ...
      this.subFormData.detailList[index].taxRate = data[0].taxRate + ''
      this.subFormData.detailList[index].taxRateName = data[0].name
      this.subFormData.detailList[index].taxRateId = data[0].id
    ...
    validateForm () {
      ...
      this.$emit('changeContractAmount', { contractAmountTax, contractAmountTaxFree })
    ...
    importSuccess (data) {
      ...
      if(data){
        this.subFormData.detailList = [...this.subFormData.detailList, ...data]
      }
    },
    /** 导出  */
    async exportFile () {
      ...
      if(!rowId){
        this.$message.info("请先暂存数据才可以进行导出");
        return
      }
