    ...
    <div class="f2bpm-online-subtable-divcontainer">
      <el-table :key="subtableKey" :data="tempRows" class="f2bpmui-datagrid-vuetable" border show-summary  :summary-method="getSummaries1">
        ...
        <el-table-column label="累计核量">
          <template slot-scope="scope">
            <amount-input v-model="scope.row.theCumulativeNuclear" size="small" placeholder="请输入" disabled />
          </template>
        </el-table-column>
        <el-table-column label="单价(含税)">
          <template slot-scope="scope">
            <amount-input v-model="scope.row.unitPriceTax" size="small" placeholder="自动计算" @change="changeAmountCount($event,scope.$index)"/>
          </template>
        </el-table-column>
        <el-table-column label="单价(无税)">
          <template slot-scope="scope">
            <amount-input v-model="scope.row.unitPriceUntax" size="small" placeholder="自动计算" disabled />
          </template>
        </el-table-column>
...
import {getProfessionContractDetail} from '@/Api/contract/index.js'
  export default {
    name: 'f2bpmSubtable',
  ...
  methods: {
    ...
    getSummaries1(param){
      ...
      columns.forEach((column, index) => {
        ...
        if (!values2.every(value => isNaN(value))) {
          sums[10] = values2.reduce((prev, curr) => {
            const value = Number(curr);
            if (!isNaN(value)) {
              return prev + curr;
            } else {
          ...
          sums[10] = Object.numberFixed(sums[10],2);
        } else {
          sums[10] = '';
        }
    ...
    changeAmountCount(e, index){
      ...
      var rate = that.getRate?that.getRate:'';
      var amount2 = 0;
      var amount3 = 0;
      var amount4 = 0;
      var amount5 = 0;
      // 金额(含税)
      amount2 = Number(price) * Number(num); // 单价(含税)*数量
      amount2 = Object.tofixed(amount2,2);
      that.$set(that.tempRows[index], 'amountTax', amount2); // 金额（含税）
      if(amount2){
        amount3 = Number(amount2)/(1+Number(rate));
        amount3 = Object.tofixed(amount3,2);
        that.$set(that.tempRows[index], 'amountUntax', amount3); // 金额（无税）
      }
      if(amount3){
        amount4 = Number(amount2)-Number(amount3);
        amount4 = Object.tofixed(amount4,2);
        that.$set(that.tempRows[index], 'taxAmount', amount4); // 税额
      }
      if(price){
        amount5 = Number(price)/(1+Number(rate));
        amount5 = Object.tofixed(amount5,2);
        that.$set(that.tempRows[index], 'unitPriceUntax', amount5); // 单价（无税）
      }
      that.$nextTick(() =>{
        that.$emit('getTableDetail',that.rows);
      })
    ...
    // 获取结算信息
    async getTeamContractDetailFn(contractOutMainId,rate){
      var that = this;
      that.getRate = rate;
      let res = await getProfessionContractDetail(contractOutMainId);
      if(res.code === 200){
        var list = res.data;
        if (list && list.length > 0) {
          $.each(list, function (index, row) {
            ...
            if(amount){
              amount2 = Number(amount)/(1+Number(rate));
              amount2 = Object.tofixed(amount2,2)
            }
            row.amountTax = amount; // 金额 （含税）
            row.amountUntax = amount2; // 金额（无税）
            if(amount2){
              amount3 = Number(amount)-Number(amount2);
              amount3 = Object.tofixed(amount3,2)
            }
            row.taxAmount = amount3; // 税额
            if(price){
              amount4 = Number(price)/(1+Number(rate));
              amount4 = Object.tofixed(amount4,2)
            }
            row.unitPriceUntax = amount4; // 单价（无税）
          });
        }
        that.rows = list;
        that.$nextTick(() =>{
