<template>
  <div class="leaseTable">
    <div >
      <el-table :data="tableData_1" v-loading="loading" style="width: 100%" v-bind:header-cell-style="{background:'#f7f7f7',color:'#000000',fontWeight:'normal'}" >
        ...
        <el-table-column label="实际租金(元)" >
          <template slot-scope="scope">
            <span>{{scope.row.actualRent | qianfenWeiFormat}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="合同占目标比" >
          <template slot-scope="scope">
            <span >{{ scope.row.ratioContract }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="已付占比" >
          <template slot-scope="scope">
            <span >{{ (Object.toDecimal(scope.row.proportionpaid,2)) }}</span>
          </template>
        </el-table-column>

        <el-table-column label="未付金额(元)" >
          <template slot-scope="scope">
            <span >{{(Object.toDecimal(scope.row.unpaidamount,2))  | qianfenWeiFormat}}</span>
          </template>
        </el-table-column>
