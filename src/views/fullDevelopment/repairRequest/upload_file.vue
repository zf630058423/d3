  ...
  <div>
    ...
    <ul class="el-upload-list el-upload-list--text">
      <li v-for="f in showData" tabindex="0" class="el-upload-list__item is-success"><a @click="handlePreview(f)"> <i class="el-icon-document"></i> {{f.name}}</a>
        <label class="my-el-upload-list__item-status-label"><i @click="formDownloadFile(f)" class="icon icon-xiazai pl5 pr5  cur blue" title="下载附件"></i><i v-if="isReadOnly" @click="deleteFile(f,showData)" class="fa fa-trash-o  ml5 pl5 pr5 cur orange" title="删除附件"></i></label>
      </li>
    </ul>
    ...
    methods: {
      ...
      handleRemove(file, fileList) {
        ...
        if(fileList  && fileList.length>0){
          that.value = fileList.filter(a => a.filePathName != filePathName);          
        }else{
...
.el-upload-list{
  ...
  .el-upload-list__item{
    ...
    align-items: center;    
    a{display: inline-block;max-width: 60%;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis; }
  } 
  .tip{ font-size: 12px; line-height: 24px;}
}
