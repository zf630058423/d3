...
<script>
  export default {
    ...
    created () {
      ...
      dg.SortOrder = 'desc';
      // 获取列表数据的接口路径  很重要
      dg.Url = '/targetcostadjustment/queryTargetcostadjustment';
      // 是否显示序号
      dg.ShowNumbers = true;
    ...
    methods: {
      ...
      getGridData () {
        ...
        // that.formData.procInstState = that.stateList.length === 0?'':that.stateList;
      ...
      handelDelete(){
        ...
        FUI.Window.confirm('是否删除所选数据？', null, function (r) {
          //删除接口
          if(rows.length>0){
            $.extendAjax("/targetcostadjustment/delTargetcostadjustment", JSON.stringify(arr), "delete", function(res) {
              if (res.code === 200) {
                that.$message({
                  message: "删除成功! ",
                  type: "success",
