<template>
  <div style="width: 100%;min-width: 400px">
    ...
    <el-table :data="tableData" style="width: 100%;" row-key="id" :key="tableKey" border stripe size="small"
              ...
              :show-overflow-tooltip="true" :load="load" :tree-props="{children:'details'}">

      <el-table-column v-if="enableSel" type="selection" width="55"></el-table-column>
      <!--<el-table-column label="序号" type="index" width="50"></el-table-column>-->
      <el-table-column label="费用项目">
        <template slot-scope="scope">
          <el-input style="width: 100px" v-model="scope.row.expenseItemType" size="small" placeholder="请输入" v-if="scope.row.pid"
      ...
      <el-table-column label="合同金额(元)" class="is-required">
        <template slot-scope="scope">
          <el-input style="width: 80%" type="number" v-model="scope.row.norigsignmny" size="small" placeholder="请输入"
                    :disabled="!scope.row.edit"></el-input>
      ...
      <el-table-column label="调整后成本(元)" class="is-required">
        <template slot-scope="scope">
          <el-input style="width: 80%" type="number" v-model="scope.row.costAfterAdjustment" size="small" placeholder="请输入"
                    :disabled="!scope.row.edit"></el-input>
          <!--<span v-else>{{scope.row.targetCost}}</span>-->
        </template>
      </el-table-column>
      <el-table-column label="累计调整成本(元)" class="is-required">
        <template slot-scope="scope">
          <el-input style="width: 80%" type="number" v-model="scope.row.cumulativeAdjustment" size="small" placeholder="请输入"
                    :disabled="!scope.row.edit"></el-input>
          <!--<span v-else>{{scope.row.targetCost}}</span>-->
        </template>
      </el-table-column>
      <el-table-column label="支出合同金额(含税)(元)" class="is-required">
        <template slot-scope="scope">
          <el-input style="width: 80%" type="number" v-model="scope.row.expenditureAmount" size="small" placeholder="请输入"
                    :disabled="!scope.row.edit"></el-input>
          <!--<span v-else>{{scope.row.targetCost}}</span>-->
        </template>
      </el-table-column>
      <el-table-column label="结算金额(含税)(元)" class="is-required">
        <template slot-scope="scope">
          <el-input style="width: 80%" type="number" v-model="scope.row.settlementAmount" size="small" placeholder="请输入"
                    :disabled="!scope.row.edit"></el-input>
          <!--<span v-else>{{scope.row.targetCost}}</span>-->
        </template>
      </el-table-column>
      <el-table-column label="累计付款(含税)(元)" class="is-required">
        <template slot-scope="scope">
          <el-input style="width: 80%" type="number" v-model="scope.row.totalPayment" size="small" placeholder="请输入"
                    :disabled="!scope.row.edit"></el-input>
          <!--<span v-else>{{scope.row.targetCost}}</span>-->
        </template>
      </el-table-column>
          ...
          if (arr[i].id === row.pid) {
            ...
            * */
            ...
            for (var j = 0; j < child.length; j++) {
              sum1 += Number(child[j].norigsignmny);
              sum2 += Number(child[j].targetCost);
              sum3 += Number(child[j].costAfterAdjustment);
              sum4 += Number(child[j].cumulativeAdjustment);
              sum5 += Number(child[j].expenditureAmount);
              sum6 += Number(child[j].settlementAmount);
              sum7 += Number(child[j].totalPayment);
            }
            that.$set(arr[i], 'norigsignmny', sum1);
            that.$set(arr[i], 'targetCost', sum2);
            that.$set(arr[i], 'costAfterAdjustment', sum3);
            that.$set(arr[i], 'cumulativeAdjustment', sum4);
            that.$set(arr[i], 'expenditureAmount', sum5);
            that.$set(arr[i], 'settlementAmount', sum6);
            that.$set(arr[i], 'totalPayment', sum7);
          }
        }
      },
      delRow(row, index) {
