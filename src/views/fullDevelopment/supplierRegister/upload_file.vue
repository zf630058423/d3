<template>
  <div>
    <el-upload name="fileUpload" class="upload-demo" :action="postUrl" :data="parms" :accept="accept" :headers="headers"
               :on-success="handleSuccess" :on-change="handleLimit" :multiple="isMultiple" :limit="limit"
    ...
    <div style="max-width:400px;padding-left: 5px;">
      <ul class="el-upload-list el-upload-list--text" style="margin-top:10px;">
        <li v-for="f in showData" tabindex="0" class="el-upload-list__item is-success">
          <a> <!--@click="handlePreview(f)"-->
            <i class="el-icon-document"></i>
            {{f.name}}
          </a>
          <label class="my-el-upload-list__item-status-label">
            <i v-if="isReadOnly" @click="formDownloadFile(f)" class="icon icon-xiazai pl5 pr5  cur blue" title="下载附件"></i>
            <i @click="deleteFile(f)" class="fa fa-trash-o  ml5 pl5 pr5 cur orange" title="删除附件"></i></label>
        </li>
      </ul>
    </div>
  </div>
...
<script>
  export default {
    name: "f2bpm-file",
    components: {},
    props: ['fieldvalue', 'rowindex', 'colindex', 'isReadOnly', 'tip', 'accept'],
    data: function () {
      return {
        isView: false,
        field: {},
        ...
        ctrType: '',
        postUrl: __serverpath + FUI.Handlers.supplierOutsideLink + 'uploadFile/',
    ...
    created: function () {
      var that = this;
      var author =JSON.parse(window.localStorage.f2bpmAuthor);
    ...
    methods: {
      ...
      handleRemove(file, fileList) {
        console.log(file,'fasfadfd');
        var that = this;
        var filePathName = file.filePathName;
        that.showData = that.showData.filter(a => a.filePathName != filePathName);
        console.log(that.showData,'12341234')
        that.$emit('showFile', that.showData);
        that.reloadShowFileList();
      },
      ...
      reloadShowFileList() {
        var that = this;
        var showData = [];
        $.each(that.showData, function (index, item) {
      ...
      formDownloadFile(file) {
        ...
        var data = Object.toAjaxDownloadFile(actionUrl, 'downloadFile', {
          'filePath': filePath,
          'fileName': fileName,
          'groupName': groupName
        });
