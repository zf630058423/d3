    ...
    <el-table :data="tableData" style="width: 100%;" row-key="id" :key="tableKey" border stripe size="small"
      ...
      <el-table-column label="地区名称">
        <template slot-scope="scope">
          <div><areaCascselect @updatevalue="updatevalue" :rowindex="scope.$index"></areaCascselect></div>
          <el-input style="width: 80%" v-model="scope.row.Region" size="small" placeholder="请输入" v-show="false"></el-input>
          <el-input style="width: 80%" v-model="scope.row.RegionId" size="small" placeholder="请输入" v-show="false"></el-input>
        </template>
      </el-table-column>
...
<script>
  ...
  import areaCascselect from '../areaCascselect'
  export default {
    name: "coverageArea",
    components: {areaCascselect},
    ...
    methods: {
      ...
      setData(){
        ...
        if(array.length === 0){
          FUI.Window.showMsg('至少填写一条业务覆盖地区表格信息');
          return isOk
        }else{
          for(let i in array){
            if(!array[i].Region){
              FUI.Window.showMsg('业务覆盖地区列表中，'+(Number(i)+1)+'行中地区名称不能为空');
              return isOk
            }
          }

        }
        this.$emit('showData', 2,array);
      },
      updatevalue(val,rIndex){
        console.log(val,'五年级',rIndex)
        if(val){
          this.tableData[rIndex].Region = val;
        }
      }
