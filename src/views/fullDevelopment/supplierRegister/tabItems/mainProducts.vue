    ...
    <el-table :data="tableData" style="width: 100%;" row-key="id" :key="tableKey" border stripe size="small"
      ...
      <el-table-column label="产品名称">
        <template slot-scope="scope">
          <el-input  type="textarea" size="small" placeholder="请输入"
                     :autosize="{ minRows: 2, maxRows: 4}"
                      style="width: 80%" v-model="scope.row.ProductName" ></el-input>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      setData(){
        ...
        if(array.length === 0){
          FUI.Window.showMsg('至少填写一条主要生产或销售产品表格信息');
          return isOk
        }else{
          for(let i in array){
            if(!array[i].ProductName){
              FUI.Window.showMsg('主要生产或销售产品列表中，'+(Number(i)+1)+'行中产品名称不能为空');
              return isOk
            }
          }

        }
        this.$emit('showData', 3,array);
      },
      // 接收父级回传数据
      getObj(arr){
        var that = this;
