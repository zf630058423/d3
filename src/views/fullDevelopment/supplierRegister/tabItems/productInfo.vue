    ...
    <el-table :data="tableData" style="width: 100%;" row-key="id" :key="tableKey" border stripe size="small"
      ...
      <el-table-column label="主打强项">
        <template slot-scope="scope">
          <el-input style="width: 80%" v-model="scope.row.MainStrength" size="small" placeholder="请输入"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="打样时间(天数)" class="is-required">
        <template slot-scope="scope">
          <el-input style="width: 80%" type="number" v-model="scope.row.SamplingTime" size="small" placeholder="请输入"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="设计风格" class="is-required">
        <template slot-scope="scope">
          <el-input style="width: 80%" type="text" v-model="scope.row.DesignStyle" size="small" placeholder="请输入"></el-input>

        </template>
      </el-table-column>
      <el-table-column label="是否专利产品" class="is-required">
        <template slot-scope="scope">
      ...
      <el-table-column label="特殊工艺" class="is-required">
        <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 返回表格数据
      setData(){
        this.$emit('showData', 5, this.tableData)
      }
    },
    mounted() {
      this.initData();
