<template>
  <div class="online-cascader">
    <el-cascader size="mini" ref="myCascader" v-if="!isReadOnly" style="width:35px;" v-model="calcValue" class="online-select" @change="change" :props="props"></el-cascader>
    <el-tooltip
      placement="bottom"
      :disabled="isShowTip"
      ...
      <el-input size="mini" class="online-cascader-input" ref="cascaderInput" :id="id" v-model="value" :disabled="!isEnableEdit" :field="field.vModel" style="width:80%;padding:0px" :clearable="clearable" :placeholder="field.fieldRemark" @mouseover.native="inputOnMouseOver($event)"></el-input>
    </el-tooltip>
  ...
  export default {
    ...
    methods: {
      ...
      lazyLoad(node, callback) {
        ...
        var parms = { 'casccode': casccode, 'parentValue': nodeData.value,'secretKey': code };
        var res = Object.toAjaxJson( FUI.Handlers.supplierOutsideLink, 'getCasSelectListJson', parms, true, null, function (data) {
      ...
      change(expandArr) {
        ...
        //that.calcValue = [];
        console.info(that.calcValue.toString());
        that.onBlur();
      },
      onBlur() {
        var that = this;
        ...
        this.$emit('updatevalue', that.value, that.rowindex);
      },
      inputOnMouseOver(event){
        const target = event.target;
        let dWidth = this.$refs.cascaderInput.$el.clientWidth;
