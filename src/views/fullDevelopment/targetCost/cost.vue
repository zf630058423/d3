    ...
    <el-form label-width="120px"
      ...
      <div class="costInfo">
        ...
        <div class="formData" style="padding:0 15px">
          <tree-table ref="treeTab" @showData="showData" @changeHeight="setFormIframeHeight" :formAction="this.$route.query.formAction" :isAble="isAble"></tree-table>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getSerialNumber() {
        var that = this;
        let res = Object.toAjaxJson(FUI.Handlers.FuiFrameworkHandler, 'getSerialNumber', {'counterCode': 'BPM_MuBiaoChengBen'});
        if (res.success) {
      ...
      submitForm() {
        ...
        // 调用我们自己的表单保存接口
        // setTimeout(function () {
        //   $.extendAjax("/objectcost/addObjectcost", JSON.stringify(that.data), "post", function(res) {
        //       if (res.code === 200) {
        //         console.log(res);
        //       } else {
      ...
      setObj(){
        var that = this;
        var name = 'ObjectCost';
        var subName = 'TargetCostInformation';
        that.data.wiid = that.wiid;
        that.data.formId = window.parent.myvm.wfContext.FormId;
        that.data.businessKey = window.parent.myvm.wfContext.BusinessKey;
        ...
        arr6.forEach(function (item) {
          var aa = {
            ...
            [name+'.'+subName+'.ExpenseItemType']:item.expenseItemType,
            [name+'.'+subName+'.Id']:item.id,
            [name+'.'+subName+'.MyId']:item.myId,
