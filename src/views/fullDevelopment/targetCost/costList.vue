        ...
        <div class="search-box" style="padding:10px 10px 0;margin: 0 20px">
          <el-form :model="formData" inline ref="form">
            <el-row :gutter="24" v-if="isOpen" class="transparent">
              <el-form-item label="项 目 部：" prop="projectName">
              ...
              <el-form-item label="编 辑 人：" prop="createName">
              ...
              <el-form-item label="编制时间：" prop="createTime">
                <el-date-picker size="small" v-model="formData.createTime" type="datetimerange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="yyyy-MM-dd HH:mm:ss" :default-time="['00:00:00', '23:59:59']"></el-date-picker>
              </el-form-item>
              <el-form-item label="单据状态：" prop="procInstState">
                <el-select v-model="formData.procInstState" placeholder="请选择" size="small">
                  <el-option
                    v-for="item in ['1','0','2','3','99']"
                    :key="item"
                    :label="item | procInstStateType"
                    :value="item">
                  </el-option>
                </el-select>
              </el-form-item>
            ...
            <el-row :gutter="24">
                <el-form-item label="单据编号：" prop="billNo">
                  <el-input class="inputWidthNum" size="small" placeholder="请输入单据编号" v-model="formData.billNo">
                ...
                <el-form-item label="施工单位：" prop="constructionUnit">
                  <el-input class="inputWidthNum" size="small" placeholder="请输入施工单位" v-model="formData.constructionUnit">
...
export default {
  ...
  created () {
    ...
    var dg = FUI.DataGrid.Create(that, 'ct_bpm_objectCost', 'ct_bpm_objectCost', 'grid', true, true, true);
    ...
    dg.SortOrder = 'desc';
    // 获取列表数据的接口路径  很重要
    dg.Url = '/objectcost/queryPageObjectcost';
    // 是否显示序号
    dg.ShowNumbers = true;
  ...
  methods: {
    ...
    handelDelete(){
      var that = this;
      // var rows=that.dg.getCheckedRows(); // 获取行数据
      var rows= that.dg.getChecked('myId'); // 获取列数据
      if(!rows){
      ...
      FUI.Window.confirm('是否删除所选数据？', null, function (r) {
        //删除接口
        if(rows.length>0){
          $.extendAjax("/objectcost/delObjectcost", JSON.stringify(arr), "delete", function(res) {
            if (res.code === 200) {
              that.$message({
                message: "删除成功! ",
                type: "success",
