...
<script>
  export default {
    ...
    created() {
      ...
      const wid = Object.toQueryString('wid');
      that.appId = appId;
      that.wid=wid;
      const currUserId = FUI.Sys.getAuthor().userId;
      const tenantId = FUI.Sys.getAuthor().tenantId;
      var dg = FUI.DataGrid.Create(that, 'f2bpm_simulation_params', 'f2bpm_simulation_params', 'grid', true, true, true);
      dg.SortName = 'id';
      dg.SortOrder = 'desc';
      dg.ShowNumbers = true;
      dg.ExpansMore = false;
      dg.DefaultWhere = "app_id='" + appId + "' s_and tenantid='" + tenantId + "'";
      ...
      dg.EditUrl = __webpath + '/workflow/simulationParams/simulationParamsEdit/?appId=' + appId+"&wid="+that.wid;
      dg.EditTitle = '仿真参数';
      dg.Bind();
    },
    mounted() {
