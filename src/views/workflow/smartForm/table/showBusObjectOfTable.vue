...
<script>
  window.refTypeEditor = FUI.Tool.createrComboboxEditor([{ 'text': '多记录', 'value': 'hasMany' }, { 'text': '单条记录', 'value': 'hasOne' }])
  export default { data() {
      return {
        // fui的dg对象
        dg: {},
    ...
    computed: {},
     created() {
      ...
      var $vm = this
      var dg = FUI.DataGrid.Create($vm, 'key_BusObjectInnerJoinTableDefinition', '', 'grid', false, false)
      dg.SortName = 'OrderNo'
      dg.SortOrder = 'ASC'
      dg.ShowNumbers = true
      dg.DefaultWhere = " RefTableId='" + tableId + "' "
      // 是否开启双击编辑
