    ...
    <el-form ref="formRef" :model="data" class="f2bpm-editform" label-width="120px">
      ...
      <el-form-item label="上传Xml文件">
        <el-upload ref="upload" name="fileUpload" :limit="1" :headers="headers" :action="postUrl" :multiple="false" :data="data" accept=".xml" :before-upload="beforeUpload" :on-success="onSuccess" :auto-upload="false" :on-error="onError">
          ...
          <div slot="tip" class="el-upload__tip">只能上传“流程设计Xml”文件</div>
        </el-upload>
      </el-form-item>
      <div style="line-height: 25px;font-size: 12px;padding-left: 30px; ">
        <em>重要说明：</em><br>
        【原样导入】 不改变任何ID键值原样导入，如果已存在相同流程或ID则导入失败；可应用于数据迁移中导入。<br>
        【复制导入】将修改所有ID键的值，使用复制新增方式导入， 可应用于多租户间导入； <span class="red">导入后需要重新绑定“表单设计”</span><br>
      </div>
...
<script>
  export default {
    data() {
      return {
        postUrl: __serverpath + FUI.Handlers.WorkflowManageHandler + 'uploadImportProcessAppXml',
        headers: {},
        data: {
          deployType: 'OriAdd'
        }
