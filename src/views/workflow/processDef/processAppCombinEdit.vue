<template>
  <div class="f2bpm-page-dialog-layout  f2bpm-page-right">
    ...
    <el-form ref="formRef" size="small" :rules="formRules" :model="data" class="f2bpm-editform" label-width="120px">
      <div  v-if="formAction=='Add'"   style="padding-left:120px;padding-bottom: 10px;padding-top: 5px;"><span class="blue cur"  title="点击跳转到批量添加页面，可批量添加多个表单与指定的共用流程组成流程应用。"  @click="goProcessAppCombinBathAdd">我要批量添加<i class="font14 icon icon-Rightarrow"></i> </span>  </div>
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="流程应用ID" prop="appId">
            <el-input v-model="data.appId" :disabled="formAction!='Add'&&!isCanModifyAppId" placeholder="长度只能由2位或4位字母组成，建议使用两位大写英文字母，流程应用ID无需一定要有意义，仅作为英文唯一值ID。" />
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="表单KEY">
            ...
            <span v-if="data.formKey!='NoneForm'&&data.formKey.indexOf('UrlForm_')==-1" title="打开表单设计" class="fa fa-cog cur   ml5 mr5 font10" @click="openOnlineFormDesigner">表单设计</span>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (formAction != FUI.FormAction.Add) {
          ...
          if (res.success) {
            ...
            that.isCanModifyWf = that.appIdInstCount == 0;
      ...
      submitForm(isGoNext) {
        ...
        if (that.data.appId.length != 2 && that.data.appId.length != 3 && that.data.appId.length != 4) {
          FUI.Window.showMsg(that, '流程应用ID长度只能是由2位至4位的英文组成，无需一定要有意义，仅作为流程应用的唯一值ID使用。');
          return false;
        }
        ...
        FUI.Form.submitForm(that, that.data, FUI.Handlers.WorkflowManageHandler, 'saveProcessApppCombin', formAction, keyId, parentGridId,
          null, null, null, null,
          function (data) {
            if (data.success) {
      ...
      goProcessAppCombinBathAdd(){
        window.location.href = __webpath + '/workflow/processDef/processAppCombinBathAdd/?parentGridId=grid&formAction=Add&mainTable=f2bpm_ProcessApp';
