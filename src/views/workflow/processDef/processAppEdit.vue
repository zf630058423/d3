  ...
  <div class="f2bpm-page-dialog-layout  f2bpm-page-right">
    ...
    <el-form ref="formRef" size="small" :rules="formRules" :model="data" class="f2bpm-editform" label-width="120px">
      ...
      <el-row>
        ...
        <el-col :span="12">
          <el-form-item label="流程名称国际化" prop="appAlias">
            <el-input v-model="data.appAlias" placeholder="流程名称国际化英文" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="12">
          <el-form-item label="流程应用ID" prop="appId">
            <el-input v-model="data.appId" :disabled="!isCanModifyAppId" placeholder="长度只能由2至4位字母组成，建议使用两位大写英文字母，流程应用ID无需一定要有意义，仅作为英文唯一值ID。" />
            <span v-if="formAction=='Add'" style="font-size: 10px;">（产生流程实例之后不可修改）</span><span v-if="formAction=='Edit'" style="font-size: 10px;" class="cube bg-gree">已发起流程实例数：{{ appIdInstCount }}</span>
            <div class="red font10" v-if="data.formKey!='NoneForm'&&formAction=='Edit'&&oldAppId!=data.appId"> 请注意：您正在修改“流程应用ID"由原来的【{{oldAppId}}】修改为【{{data.appId}}】，修改后需要重新设置“表单字段权限”，请谨慎操作！！！</div>
          </el-form-item>
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="表单应用" prop="formKey">
            ...
            <span style="font-size: 10px;"><span v-if="isCanModify" title="是指只使用流程不使用表单，运行时无在线表单，只使用流程的审批流转。">无需表单
                <el-switch v-model="isNoneProcessForm" size="mini" active-color="#87DC48" inactive-color="#DEDEDE" @change="changeIsNoneProcessForm" />
              </span> <span v-if="!isNoneProcessForm">（产生流程实例之后不可修改）</span></span>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="表单KEY">
            <span class="cube bg-blue" v-html="data.formKey" />
            <span v-if="formAction!='Add'&&data.formKey!='NoneForm'&&data.formKey.indexOf('UrlForm_')==-1" title="打开表单设计" class="fa fa-cog cur   ml5 mr5 font10" @click="openOnlineFormDesigner">表单设计</span>
            <span v-if="formAction!='Add'&&data.formKey!='NoneForm'&&data.formKey.indexOf('UrlForm_')==-1" title="打开表单设计" class="fa fa-cog cur   ml5 mr5 font10" @click="editFormPermissions">表单字段权限设置</span>
          </el-form-item>
        </el-col>
      </el-row>
      ...
      <el-row>
        ...
        <el-col v-if="!isNoneProcessForm" :span="4">
        ...
        <el-col v-if="!isNoneProcessForm" :span="6">
          <el-form-item label="结束时可看节点表单">
            <el-switch v-model="data.isViewActForms" active-color="#87DC48" inactive-color="#DEDEDE" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-form-item label="流程应用编码" prop="appCode">
        <el-input v-model="data.appCode" placeholder="自定义流程应用编码" />
      </el-form-item>
      ...
      <div class="dialogfooter2">
        ...
        <el-button v-if="formAction=='Add'" size="small" type="primary" @click="submitForm(1)"><i class="fa fa-save" />保存
        </el-button>
  ...
  export default {
    ...
    mounted() {
      ...
      if (formAction == FUI.FormAction.Add) {
        that.loadNewAppId();
        that.data.workflowKey = "WF" + Object.toGetCurrentDateTimeMillisecond();
      }
      that.loadTreeSelect();
      if (that.keyId) {
        that.loadData();
    ...
    , methods: {
      ...
      changeIsNoneProcessForm(v) {
        ...
        if (v) {
          ...
          that.data.isViewActForms=false;
        } else {
          that.data.formKey = '';
          that.data.formName = '';
        }
      ...
      loadData() {
        ...
        if (formAction != FUI.FormAction.Add) {
          ...
          if (res.success) {
            ...
            that.isCanModifyAppId = that.appIdInstCount == 0;
            if (!that.isNoneProcessForm) {
      ...
      submitForm(isGoNext) {
        ...
        if (that.data.appId.length != 3 && that.data.appId.length != 2 && that.data.appId.length != 4) {
          FUI.Window.showMsg(that, '流程应用ID长度只能是由2至4位的英文组成，无需一定要有意义，仅作为流程应用的唯一值ID使用。');
          return false;
        }
          ...
          function (data) {
            if (data.success) {
              ...
              FUI.Window.showMsg(that, data.msg, null, 1, function () {
                if (isGoNext == 1) {
                  that.goProcessDefListByWorkflowKey();
                } else {
      ...
      // 流程应用编辑
      goProcessAppEdit() {
        window.location.href = __webpath + '/workflow/processDef/processAppEdit/?parentGridId=grid&formAction=Edit&mainTable=f2bpm_ProcessApp&keyId=' + this.data.appId;
      },
      // 流程定义
      goProcessDefListByWorkflowKey() {
        var that = this;
        window.location.href = __webpath + '/workflow/processDef/processDefListByWorkflowKey/?appId=' + that.data.appId + '&workflowKey=' + this.data.workflowKey;
      },
      editFormPermissions() {
        var that = this;
