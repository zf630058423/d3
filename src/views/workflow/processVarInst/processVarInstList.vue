  ...
  <div class="f2bpm-page-layout">
    <el-container>
      <el-main class="f2bpm-page-right">
        ...
        <f2bpm-gridsearchform @expandclick="expandClick"
         :tableform="tableForm" @searchformbtnclick="searchFormbtnClick" @updatevalue="updateFieldValue" />
        <f2bpm-datagrid ref="grid" grid="grid" :expand-search="expandSearch"
        :toolbar="tableToolbar" :context-menu="{header: {options: headerMenus},className: 'my-menus'}" :pager-config="tablePage" :data="tableData" :columns="tableColumn" @cell-dblclick="celldbClick" @toolbar-button-click="toolbarButtonClickEvent" @header-contextmenu="contextMenuClickEvent" @page-change="pageChange" @sort-change="sortChange" />
      </el-main>
    </el-container>
  </div>
</template>
...
<script>
  export default {
    ...
    created() {
      var that = this;
      var dg = FUI.DataGrid.Create(that, 'f2bpm_ProcessVarInst', 'f2bpm_ProcessVarInst', 'grid', true, true, true);
      dg.SortName = 'Id';
      dg.SortOrder = 'desc';
      dg.ShowNumbers = true;
      dg.ExpansMore = false;
      ...
      dg.EditUrl = '/workflow/processVarInst/processVarInstEdit/';
      dg.EditTitle = '变量实例';
      dg.Bind();
    },
    mounted() {
    ...
    methods: {
      ...
      showDialogEdit(formAction, keyId) {
        ...
        FUI.Window.openEdit(that.dg.EditUrl, title, formAction, that.dg.GridId, 650, 550, null, that.dg.MainTable, keyId, null, null, false);
      },
      // 工具栏按钮事件
