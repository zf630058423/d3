  ...
  <div class="f2bpmui-datagrid-vuetable-container mytable">
    <div v-if="toolbar&&toolbar.buttons.length>0" id="grid-toolbar" class="grid-toolbar">
      <template v-for="(btn,index) in toolbar.buttons">
        <el-button v-if="buttonIsHaveShowPower(btn.code)" :key="index" :code="btn.code" size="small" 
          :type="btn.code=='default.delete'?'danger':btn.status? btn.status:btn.type" @click="toolbarClick(btn,$event)">
            <i :class="btn.icon" /><span v-html="language== 'en-US'?btn.alias: btn.name" /></el-button>
      </template>
    </div>
    <!-- row-key：之前值是PK -->
    <el-table
...
<script>
  ...
  props: {
    ...
    created() {
      ...
      that.currRealName = that.$store.state.user.author ? decodeURI(that.$store.state.user.author.realName) : '';
      // 找出序号列和复选框列
    ...
    methods: {
      ...
      autoResizeHeight() {
        var that = this;
        var theHeight = this.gridHeight || $(window).height();
        ...
        that.tableHeight = theHeight - that.clientHeight;
      },
      // 表头右键点击时弹出菜单
