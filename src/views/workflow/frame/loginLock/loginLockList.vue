...
<script>
  export default {
    ...
    created() {
      ...
      var dg = FUI.DataGrid.Create(that, 'f2bpm_sys_login_lock', 'f2bpm_sys_login_lock', 'grid', true, true, true);
      dg.SortName = 'id';
      dg.SortOrder = 'desc';
      dg.ShowNumbers = true;
      dg.ExpansMore = true;
      ...
      dg.EditUrl = '/workflow/frame/loginLock/loginLockEdit';
      dg.EditTitle = '锁定信息';
      dg.Bind();
    },
    mounted() {
    ...
    methods: {
      ...
      statusFormatter(row, column, value, index) {
        ...
        switch (value.toString()) {
          case '0':
            result =  '<span class="cube bg-grey">无效</span>';  
            break;
          case '1':
            result = '<span class="cube bg-blue">有效</span>';
            break;
          case '2':
            result = '<span class="cube bg-gree">已解锁</span>';  
            break;
        }
        return result;
      }
