...
<script>
  export default {
    ...
    created() {
      ...
      let dg = FUI.DataGrid.Create(that, 'f2bpm_sys_data_division_detail', 'f2bpm_sys_data_division_detail', 'grid', true, true, true);
      dg.SortName = 'id';
      dg.SortOrder = 'desc';
      dg.ShowNumbers = true;
      dg.ExpansMore = true;
      ...
      dg.DefaultWhere = Object.toStringFormat("ref_division_id='{0}'", divisionId);
      ...
      dg.EditUrl = '/security/workflow/dataDivisionDetail/dataDivisionDetailEdit';
      dg.EditTitle = '【数据切割记录明细】';
      dg.Bind();
    },
    mounted() {
      window.myvm = this;
    ...
    methods: {
      ...
      myOperationDeleteEvent(value, row, index) {
        ...
        FUI.Window.confirm(that, "您确定要删除吗？", null, function () {
          Object.toAjaxJson("/security/workflow/dataDivisionDetail/", "delete", parm, true, null,
            function (res) {
              if (res.success) {
                that.dataGridSearch();
                //that.refreshTree();
      ...
      bathDeleteEvent() {
        ...
        FUI.Window.confirm(that, "您确定删除记录吗？", null, function () {
          Object.toAjaxJson("/security/workflow/dataDivisionDetail/", "deleteList", parm, true, null,
            function (res) {
              if (res.success) {
                that.dataGridSearch();
              } else {
