...
<script>
  ...
  export default {
    ...
    created() {
      var that = this;
      window.myvm = that;
      var dg = FUI.DataGrid.Create(that, 'vw_orgImpl_RoleInResource', 'sys_RoleInResource', 'grid', true, true, true);
    ...
    methods: {
      ...
      onTreeSelected(data, treeNode, thisObj) {
        ...
        this.currRoleName = treeNode.data.title;
        this.dataGridSearch();
      },
      // 刷新列表
      ...
      myToolBarCustomButton() {
        var btns = [];
        btns.push({ code: 'addResource', status: 'default', name: '添加菜单授权', icon: 'fa fa-gamepad' });
        return btns;
      },
      // 工具栏按钮事件
      ...
      rresTypeFormatter(row, column, value, index) {
      ...
      addResource() {
        var that = this;
        var roleId = that.searchParams.RoleId;
        var roleName = that.currRoleName;
        if (roleId == '' || roleId == '00000000-0000-0000-0000-000000000000') {
          FUI.Window.showMsg2('请在左侧列表中选择角色');
          return false;
        }
        FUI.Window.openEdit(FUI.Handlers.AdminHandler + 'role/roleInResourceEdit/?rresType=orgImpl&roleId=' + roleId + '&roleName=' + roleName, '为接入角色【<em>' + roleName + '</em>】分配菜单', FUI.FormAction.Edit, 'grid', 400, 500, null, null, null, true);
      }
    }
