  ...
  export default {
    ...
    methods: {
      load() {
        ...
        var url = __webpath + '/workflow/security/article/articleEdit/?isLink=true&infoType=' + infoType + '&functionID=' + functionID;
        var fastChannelUrl = __webpath + '/workflow/security/article/fastChannelEdit/?isLink=true&infoType=' + infoType + '&functionID=' + functionID;
        dg.EditUrl = functionID == 'FastChannel' ? fastChannelUrl : url;
        dg.EditTitle = '链接';
        dg.Bind();
      },
      collapsedSider() {
      ...
      showDialogEdit(formAction, keyId) {
        ...
        FUI.Window.openEdit(that.dg.EditUrl, title, formAction, that.dg.GridId, 750, 550, null, that.dg.MainTable, keyId);
      },
      // 工具栏按钮
      ...
      myOperationCustomButton(h, value, row, index) {
        var custBtns = [];
        var item = { index: 90, title: '发布范围', alias: 'Scope of release', tip: '设置发布授权范围', class: 'fa fa-cogs', code: 'custom', click: () => this.publicAuthor(row.ArticleId, row.ArticleTitle) };
        custBtns.push(item);
        return custBtns;
      },
