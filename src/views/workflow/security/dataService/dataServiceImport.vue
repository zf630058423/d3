    ...
    <el-form ref="formRef" :model="data" class="f2bpm-editform" label-width="120px">
      ...
      <el-form-item label="上传Xml文件">
        ...
        >
          ...
          <div slot="tip" class="el-upload__tip">只能上“数据服务定义Xml”文件</div>
        </el-upload>
      </el-form-item>
      <div style="line-height: 25px;font-size: 12px;padding-left: 30px; ">
        <em>导入方式说明：</em> 
        <br>【原样导入】不改变任何ID值原样导入，如果已存在相同的数据服务则导入会失败;可应用于数据迁移中导入。
...
<script>
  export default {
    data() {
      return {
        postUrl: __serverpath +   '/workflow/security/dataService/uploadImportDataServiceXml',
        headers: {},
         data: {
          deployType: 'OriAdd'
        }
