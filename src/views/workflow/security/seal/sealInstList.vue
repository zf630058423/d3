...
<script>
  export default { data() {
    ...
    computed: {
     ...
     created() {
      var that = this
      var dg = FUI.DataGrid.Create(that, 'f2bpm_sys_SealInstance', 'f2bpm_sys_SealInstance', 'grid', true, true, true)
      dg.SortName = 'LogTime'
      dg.SortOrder = 'desc'
      dg.ShowNumbers = true
      dg.ExpansMore = false
      ...
      dg.EditUrl = FUI.Handlers.SealController + 'sealInstEdit/'
      dg.EditTitle = '印章实例'
      dg.Bind()
    },
     mounted() {
    ...
    methods: {
      ...
      // 操作列自定义按钮
       myOperationCustomButton(h, value, row, index) {
        var custBtns = []
        var item = { title: '验证记录', tip: '查看盖章被验证记录', class: 'fa fa-list', code: 'publicAuthor', click: () => this.showCheckList(row.SealId, row.SealInstanceId, row.StampDirection) }
        custBtns.push(item)
        return custBtns
      },
       showCheckList(sealId, sealInstanceId, stampDirection) {
        FUI.Window.openEdit(FUI.Handlers.SealController + 'sealCheckList/?sealId=' + sealId, '查看印章验证记录', 'View', dg.GridId, dg.EditWidth, dg.EditHeight, null, dg.MainTable, sealInstanceId, false)
      }
    }
  }
</script>
