    ...
    <el-container>
      <el-main class="f2bpm-page-right">
        ...
        <div id="grid-toolbar" class="grid-toolbar">
          <el-button size="small" type="default" @click="clearCacheByKey('workflowCache','OrganizationConfigCahe')">
            <i class="icon   icon-qingchu"></i>刷新配置缓存
          </el-button>
        </div>
...
<script>
  export default {
    ...
    created() {
      var that = this;
      var dg = FUI.DataGrid.Create(that, 'f2bpm_sys_organizationconfig', 'f2bpm_sys_organizationconfig', 'grid', true, true, true);
      dg.SortName = 'fieldType';
      dg.SortOrder = 'desc';
      dg.ShowNumbers = true;
      dg.ExpansMore = false;
    ...
    methods: {
      ...
      , clearCacheByKey(groupName, key) {
        var that = this;
        FUI.Window.confirm(that, "确定要清除缓存?", '删除提示', function (r) {
      ...
      , myOperationCustomButton(h, value, row, index) {
        ...
        var item = h('button', {
          ...
          class: 'btn btn-minier btn-primary mr2',
          on: {
