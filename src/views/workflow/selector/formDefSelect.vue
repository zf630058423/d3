  ...
  <div class="f2bpm-page-dialog-layout">
    ...
    <el-container>
      ...
      <el-main class="f2bpm-page-right" style="overflow: hidden">
        ...
        <f2bpm-datagrid ref="grid" grid="grid" :expand-search="expandSearch" :expand-subheight="100" height="full-110" :context-menu="{header: {options: headerMenus},className: 'my-menus'}" :pager-config="tablePage" @checkbox-all="selectAllEvent" :data="tableData" :columns="tableColumn" @checkbox-change="selectChangeEvent" @radio-change="selectChangeEvent" @header-contextmenu="contextMenuClickEvent" @page-change="pageChange" @sort-change="sortChange" />
      </el-main>
...
<script>
  export default {
            ...
            default: ({ row, column }, h) => {
              return [
                h('button', {
                  ...
                  on: {
                    click: () => this.delRow(row.FormDefId, 1)
                  }
                }, '')];
            }
          }
    ...
    created() {
      var $vm = this;
      $vm.loadWindowparms();
      var dg = FUI.DataGrid.Create($vm, 'key_selector_FormDef', 'f2bpm_form_FormHtml', 'grid', true, false, true);
    ...
    methods: {
      ...
      loadWindowparms() {
        const that = this;
        // 初始化选中的数据，[{ "FormDefId": "2222", "FormDefName": "55555" }]
