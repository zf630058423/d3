      ...
      <el-aside ref="sideLeft" class="f2bpm-page-left" hide-trigger collapsible :width="isCollapsed?'20px':'200px'">
        <div class="f2bpm-panel-head">
          ...
          <div style=" float: right;line-height:35px;padding-right: 5px;">
            <i v-if="!isCollapsed" title="显示所有" class="fa fa-refresh  cur ml10 font12 " @click="loadTree()" />
  ...
  export default {
    data() {
      return {
        expandSearch: "",
        formId: '',
        windowparms: { resultFieldKey: 'WorkflowKey', singleSelect: true, data: [] },
        // ======stand:标准区=============
        treeFilterText: '',
        isCollapsed: false,
        // ==data grid====
    ...
    created() {
      ...
      var dg = FUI.DataGrid.Create($vm, 'key_selector_Workflow', null, 'grid', true, true, true);
      dg.SortName = ' CreatedTime ';
      dg.SortOrder = 'desc';
      dg.IsSimpleSearch = true;
      dg.SingleSelect = $vm.windowparms.singleSelect;
      if (procInfoType) {
        dg.DefaultWhere = "procInfoType='" + procInfoType + "'";
      }
      // 是否开启双击编辑
