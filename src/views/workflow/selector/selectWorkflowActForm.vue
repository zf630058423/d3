    ...
    <div>
      <div class="fence-box" style="width: 220px;">
        <div class="fence-box-title">
          <i class="fa fa-table" aria-hidden="true" />流程节点表单（单击选择）：
        </div>
        <ul id="list">
          <li v-for="item in leftData" class="fence-li-item" :formDefId="item.formDefId" :formKey="item.formKey" :formName="item.formName" :activityid="item.activityId" :activityname="item.activityName" :activitycode="item.activityCode" @click="leftSelected(item.activityId)">
            {{ item.activityName }}-{{ item.formName }}
          </li>
        </ul>
      </div>
      <div class="fence-box right-result-boxlist" style="width: 180px;">
        <div class="fence-box-title">
          <i class="fa fa-folder" aria-hidden="true" />已选节点表单（单击删除）：
        </div>
        <ul id="listResult">
  ...
  export default { data() {
    ...
    computed: {
     ...
     created() {
      ...
      var res = Object.toAjaxJson('/workflow/selector/', 'getWorkflowActFormList', { appId: appId, wid: wid })
      if (res.success) {
