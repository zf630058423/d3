...
@checkbox-all="selectAllEvent" :data="tableData" :columns="tableColumn" @checkbox-change="selectChangeEvent" @radio-change="selectChangeEvent"
      ...
      <el-aside ref="sideRight" class="f2bpm-selector-page-right" width="180px">
        <f2bpm-datagrid ref="gridResult" grid="gridResult"  :data="resultGridData" :columns="resultGridColumn" />
      </el-aside>
    </el-container>
  </div>
</template>
...
<script>
  export default { data() {
      return {
        windowparms: { resultFieldKey: 'tenantCode', singleSelect: true, data: [] },
        // ======stand:标准区=============
        treeFilterText: '',
        isCollapsed: false,
        // ==data grid====
    ...
    watch: {
     ...
     created() {
      ...
      $vm.loadWindowparms()
      var dg = FUI.DataGrid.Create($vm, 'key_selector_tenantor', null, 'grid', true, true, true)
      dg.SortName = 'tenant_code'
      dg.SortOrder = 'ASC'
