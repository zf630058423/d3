  ...
  <div class="f2bpm-page-layout">
    <el-container>
      <el-main class="f2bpm-page-right" style="overflow: hidden">
        ...
        <f2bpm-datagrid :expand-search="expandSearch" :expand-subheight="100" ref="grid" grid="grid" height="full-110" :context-menu="{header: {options: headerMenus},className: 'my-menus'}" :pager-config="tablePage" @checkbox-all="selectAllEvent" :data="tableData" :columns="tableColumn" @checkbox-change="selectChangeEvent" @radio-change="selectChangeEvent" @header-contextmenu="contextMenuClickEvent" @page-change="pageChange" @sort-change="sortChange" />
      </el-main>
  ...
  export default {
    data() {
      return {
        ...
        expandSearch: "",
        windowparms: { resultFieldKey: 'sealId', singleSelect: true, data: [] },
        // ======stand:标准区=============
        treeFilterText: '',
        isCollapsed: false,
        // ==data grid====
    ...
    created() {
      var $vm = this;
      $vm.loadWindowparms();
      var dg = FUI.DataGrid.Create($vm, 'key_selector_Seal', null, 'grid', true, true, true);
      dg.SortName = 'sealCode';
      dg.SortOrder = 'ASC';
      dg.IsSimpleSearch = true;
      dg.SingleSelect = true;
      dg.Url = '/workflow/security/seal/loadPersonalSealListJsonByPage/?defaultWhere=' + dg.DefaultWhere;
      // 是否开启双击编辑
