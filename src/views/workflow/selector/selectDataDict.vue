  ...
  <div class="f2bpm-page-layout">
    <el-container>
      <el-main class="f2bpm-page-right" style="overflow: hidden">
        ...
        <f2bpm-datagrid :expand-search="expandSearch" ref="grid" grid="grid" height="full-110" :expand-subheight="100" :context-menu="{header: {options: headerMenus},className: 'my-menus'}" :pager-config="tablePage" @checkbox-all="selectAllEvent" :data="tableData" :columns="tableColumn" @checkbox-change="selectChangeEvent" @radio-change="selectChangeEvent" @header-contextmenu="contextMenuClickEvent" @page-change="pageChange" @sort-change="sortChange" />
      </el-main>
  ...
  export default {
    data() {
      return {
        expandSearch: "",
        configType: '',
        windowparms: { resultFieldKey: 'ConfigValue', singleSelect: true, data: [] },
        // ======stand:标准区=============
        treeFilterText: '',
        isCollapsed: false,
        // ==data grid====
    ...
    created() {
      ...
      $vm.loadWindowparms();
      var dg = FUI.DataGrid.Create($vm, 'key_selector_datadict', null, 'grid', true, true, true);
      dg.SortName = ' OrderNo ';
      dg.SortOrder = 'ASC';
      dg.IsSimpleSearch = true;
      if ($vm.configType != '') {
        dg.DefaultWhere = " (TenantId='" + currUser.tenantId + "' s_or TenantId='System') s_and configType='" + $vm.configType + "' ";
      }
    ...
    methods: {
      ...
      loadWindowparms() {
        ...
        if (parmsStr) {
          ...
          parms = eval('(' + parmsStr + ')');
          that.configType = parms.configType || '';
