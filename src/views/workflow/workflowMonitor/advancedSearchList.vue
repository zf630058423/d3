      ...
      <el-main class="f2bpm-page-right">
        <div>
          ...
          <div id="grid-toolbar" class="grid-toolbar">
            ...
            <el-button size="small" :type="searchParams.isHistory==1&&searchParams.divisionYear==''?'primary':'default'" @click="changeList(1)">
              {{$t('pageInfo.finished')}}
            </el-button>
            <el-button v-for="item in dataDivisionData" size="small" :type="searchParams.isHistory==1&&searchParams.divisionYear==item.divisionValue?'primary':'default'" @click="changeList(1,item.divisionValue)">
             {{item.divisionValue+'年'}}
            </el-button>
          </div>
          <div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.loadTree();
      this.getGridData();
      this.loadEnabledDataDivisionList();
    }
    , mounted() {
      window.myvm = this;
    ...
    methods: {
      ...
      dataGridReSet() {
        ...
        $.each(this.searchParams, function (key, item) {
          if (key != 'isHistory' && key != 'divisionYear') {
      ...
      changeList(isHistory, divisionYear) {
        this.searchParams.isHistory = isHistory;
        this.searchParams.divisionYear = divisionYear || "";
        this.dataGridSearch();
      },
      onSearchExpansMore() {
        var name = "";
      ...
      loadEnabledDataDivisionList() {
        var that = this;
        Object.toAjaxJson(FUI.Handlers.WorkflowBusinessHandler, 'getProcInstDivisionList', null, true, null, function (res) {
          that.dataDivisionData = res.data;
          console.info(that.dataDivisionData);
        }
        );
      },
