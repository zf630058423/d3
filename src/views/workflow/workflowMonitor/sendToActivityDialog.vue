  ...
  <div>
    <el-form ref="formRef" :model="data" size="small" class="f2bpm-editform" label-width="120px">
      ...
      <el-form-item label="说明">
        <span id="msg">将任务直接提交到指定的节点，不指定节点处理人时，则默认使用节点配置计算器计算出的用户为处理人。</span>
        <br><em>如果在存在并行分支，请警慎操作！</em>
...
<script>
  ...
  export default {
    ...
    created: function () {
      ...
      that.actorSelectTitle = '【' + sheetId + '】' + '提交到指定节点';
      var listActivity = Object.toAjaxJson(FUI.Handlers.WorkflowBusinessHandler, 'getCanSendListActivity', { taskId: taskId });
      ...
      submitForm() {
        ...
        FUI.Window.confirm(that, '您确定要提交吗？', '提示', function () {
          ...
          if (userIds != '' && activityCode != '') {
            var data = { taskId: that.taskId, 'userIds': userIds, 'activityCode': activityCode, 'opinion': '[调度干预]' + that.data.opinion };
            var mssg = Object.toAjaxJson(FUI.Handlers.WorkflowBusinessHandler, 'sendTaskToActivity', data);
