      ...
      <el-main class="f2bpm-page-right">
        <div>
          ...
          <div id="grid-toolbar" class="grid-toolbar">
            <el-button size="small" :type="searchParams.isHistory==0?'primary':'default'" @click="changeRunningHistoryList(0,'activationProcessList')">
              {{$t('listButton.activated') }}
            </el-button>
            <el-button size="small" :type="searchParams.isHistory==1?'primary':'default'" @click="changeRunningHistoryList(1,'activationProcessList')">
              {{$t('listButton.unActivated') }}
            </el-button>
          </div>
          <div>
  ...
  export default {
          ...
          { type: 'checkbox', width: 35, align: 'center', fixed: 'left' },
          {
            ...
            slots: {
              ...
              default: ({ row, column }, h) => {
                ...
                var that = this;
                var btns = [];
                if ((rec.workflowInstanceState == 3 || rec.workflowInstanceState == 99) && that.searchParams.isHistory == 1) {
                  var btn = h('button', {
                    attrs: { title: '重新将流程从归档中激活' },
                    class: 'btn btn-minier btn-danger mr2 fa fa-deafness',
                    on: {
                      click: () => this.activationProcess(row.workflowInstanceId, row.sheetId,row.mainWorkflowInstanceId)
                    }
                  }, '从归档中激活');
                  btns.push(btn);
                } else if ((rec.workflowInstanceState == 3 || rec.workflowInstanceState == 99) && that.searchParams.isHistory == 0) {
                  var btn = h('button', {
                    attrs: { title: '激活到指定的节点' },
                    class: 'btn btn-minier btn-primary mr2 fa fa-deafness',
                    on: {
                      click: () => this.activationToActivity(row.workflowInstanceId, row.sheetId)
                    }
                  }, '激活到节点');
                  btns.push(btn);
                  btn = h('button', {
                    attrs: { title: '再次将流程归档' },
                    class: 'btn btn-minier btn-danger mr2 fa fa-cubes',
                    on: {
                      click: () => this.divertWorkflowInstanceToHistoryTable(row.workflowInstanceId, row.sheetId)
                    }
                  }, '流程归档');
                  btns.push(btn);
                }
    ...
    methods: {
      ...
      getGridData() {
        ...
        parm.inWorkflowInstanceState = '3,99';
      ...
      changeRunningHistoryList(isHistory) {
      ...
      activationToActivity(wiid, sheetId) {
        var that = this;
        if (!wiid) {
          FUI.Window.showMsg('请选择要重新激活的流程。');
          return false;
        }
        FUI.Window.openEdit('/workflow/workflowMonitor/sendActiveToActivityDialog/?wiid=' + wiid + '&sheetId=' + sheetId, '流程 【' + sheetId + '】 重新激活提交到指定节点', 'Add', 'grid', 860, 560);
      },
      divertWorkflowInstanceToHistoryTable(wiid, sheetId) {
        var that = this;
        var msg = '确定要对工单【' + sheetId + '】进行归档吗?';
        FUI.Window.confirm(msg, '温馨提示', function (r) {
          if (r) {
            var data = {
              wiid: wiid
            };
            var result = Object.toAjaxJson(FUI.Handlers.WorkflowBusinessHandler, 'divertWorkflowInstanceToHistoryTable', data);
      ...
      activationProcess(wiid, sheetId,mainWorkflowInstanceId) {
        var that = this;
        if(mainWorkflowInstanceId){
          FUI.Window.showMsg(that,"子流程实例不允许重新激活");
          return false;
        }
        var msg = '<em>警慎操作! 警慎操作! 警慎操作!</em> 您确定要对工单【' + sheetId + '】重新从归档中激活到运行状态吗?';
        FUI.Window.confirm(msg, '温馨提示', function (r) {
          if (r) {
            var data = {
              wiid: wiid
            };
            var result = Object.toAjaxJson(FUI.Handlers.WorkflowBusinessHandler, 'activationProcess', data);
            FUI.Window.showMsg(result.msg, null, 'info', function () {
              that.refreshGrid();
            });
          }
