<template>
  <div>
    <el-form ref="formRef" :model="data" class="f2bpm-editform importOrgByExcel" label-width="120px">
      ...
      <el-form-item>
        <div class="font12">用户初始登录密码(仅对新增时有效)</div>
      ...
      <div style="line-height: 25px;font-size: 14px;padding-left: 30px; ">
        <el-tooltip class="item" effect="dark" content="新增导入用户，如果账号已存在，则会进行复盖更新。<br>导入只做增量式导入。 此功能一般用于初始化时的批量导入。" placement="right">
          <i class="el-icon-question">说明</i>；
        </el-tooltip>
      </div>
      <div style="line-height: 25px;font-size: 14px;padding-left: 30px; ">
        <em>下载用户导入模板：</em><span class="cur blue" @click="dowloadTemplate">用户导入模板.xlsx</span> <br>
      </div>
    ...
    methods: {
      ...
      , dowloadTemplate() {
        ...
        Object.toAjaxDownloadFile(downUrl, "", {
          "templateName": "UserTemplate.xlsx",
