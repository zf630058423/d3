          ...
          <div id="gridSearch" class="grid-search-form">
            <div id="grid-search-form-list" class=" grid-search-form-list  ">
              <ul>
                <li>
                  <label>账号：</label>
                  <el-input id="SheetId" v-model="searchParams.userName" size="mini" clearable class="textBoxShort" />
                </li>
                <li>
                  <label>姓名：</label>
                  <el-input id="WorkflowTitle" v-model="searchParams.realName" size="mini" clearable class="textBoxShort" />
                </li>
                <li>
                  <label>角色：</label>
...
<script>
  export default {
                ...
                return [
                  h(
                    'span',
                    {
                      ...
                      on: {
                        click: () => this.editDetail(row.userId, row.realName)
                      }
                    },
                    '自定义列表按钮权限'
                  )
    ...
    methods: {
      ...
      getGridData() {
        var that = this;
        if(that.searchParams.userName==''&&that.searchParams.realName==''){
          FUI.Window.showMsg("至少需要输入“账号”或“姓名”进行查询");
          return false;
        }
        ...
        var res = Object.toAjaxJson('/admin/userPermission/', 'getUserPermissionInfoList', parm, true, null,
          function (res) {
            if (res.success) {
      ...
      , editDetail(userId, realName) {
        FUI.Window.openEdit("/admin/user/userPermissionInfoDetail?userId=" + userId, "【" + realName + "】的自定义列表的按钮权限", "Edit", "grid", 900, 580, null, null, null, true);
      }
    }
  }
</script>
