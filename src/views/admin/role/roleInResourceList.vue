    ...
    <el-container>
      <el-aside ref="sideLeft" class="f2bpm-page-left" hide-trigger collapsible :width="isCollapsed?'20px':'250px'">
        ...
        <div v-if="!isCollapsed" class="f2bpm-page-left-content auto-window-height " :style="'height:'+countLeftTreeHeight+'px;'">
          <span class="cur font12 m10" @click="showAll()"><i class="icon icon-quanbu font14"></i> {{$t('all')}}</span>
...
<script>
  export default {
    ...
    created() {
      var that = this;
      that.loadTree();
      var dg = FUI.DataGrid.Create(that, 'vw_sys_RoleInResource', 'sys_RoleInResource', 'grid', true, true, true);
    ...
    methods: {
      ...
      myToolBarCustomButton() {
        var btns = [];
        btns.push({ code: 'addResource', status: 'default', name: '添加授权', alias: 'Authorize', icon: 'fa fa-gamepad' });
      ...
      addResource() {
        ...
        FUI.Window.openEdit(FUI.Handlers.AdminHandler + 'role/roleInResourceEdit/?roleName=' + encodeURI(that.roleName) + '&roleId=' + that.roleId, '为【<em>' + that.roleName + '</em>】角色分配权限', FUI.FormAction.Edit, 'grid', 350, 500, null, null, null, true);
      },
      exportSqlFile() {
        var that = this;
        ...
        Object.toAjaxDownloadFile(FUI.Handlers.AdminHandler + 'role/', 'downloadRoleInResourceSql', {
          ids: ids
        });
      }
    }
