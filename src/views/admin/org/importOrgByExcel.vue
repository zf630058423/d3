    ...
    <el-form ref="formRef" :model="data" class="f2bpm-editform" label-width="120px">
      <el-form-item label="上传模板">
        <el-upload :fileList="attachmentList" ref="upload" name="fileUpload" :limit="1" :headers="headers" :action="postUrl" :multiple="false" :data="data" accept=".xlsx" :before-upload="beforeUpload" :on-success="onSuccess" :auto-upload="false" :on-error="onError">
          ...
          <div slot="tip" class="el-upload__tip">只能上传组织导入Excel模板</div>
        </el-upload>
      </el-form-item>
      <el-form-item label="">
      ...
      <div style="line-height: 25px;font-size: 14px;padding-left: 30px; ">
        <el-tooltip class="item" effect="dark" content="新增导入组织; 如果组织编码已存在，则会进行复盖更新。<br>导入只做增量式导入。 此功能一般用于初始化时的批量导入。" placement="right">
          <i class="el-icon-question"> <span class="font12">说明：导入成功后需要手动点击“重构路径编码”</span></i>
        </el-tooltip>
      </div>
      <div style="line-height: 25px;font-size: 14px;padding-left: 30px;margin-top: 10px; ">
        <em>下载模板：</em><span class="cur blue" @click="dowloadTemplate">组织导入模板.xlsx</span> <br>
      </div>
...
<script>
  export default {
    data() {
      return {
        attachmentList: [],
        postUrl: __serverpath + '/admin/user/saveImportOrg',
        headers: {},
        message: "",
        data: {
          excelType: 'myexcel'
    ...
    methods: {
      ...
      onSuccess(response, file, fileList) {
        ...
        if (response.success) {
          ...
          that.attachmentList = [];
          FUI.Window.showMsg(that, '导入失败' + response.msg)
        }
      },
      ...
      , dowloadTemplate() {
        ...
        Object.toAjaxDownloadFile(downUrl, "", {
          "templateName":"OrgImportTemplate.xlsx",
