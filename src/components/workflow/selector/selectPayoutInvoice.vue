      ...
      <el-main class="f2bpm-page-right" style="overflow: hidden">
        <div id="gridSearch" class="grid-search-form">
          <div id="grid-search-form-list" :class="'grid-search-form-list ' + searchFromData.collapsedHeight">
            <ul>
              <li>
                <label class="textClass"> 购方：</label>
                <el-input v-model="searchParams.buyerName" size="mini" clearable class="textBoxShort" placeholder="请输入"/>
              </li>
              <li>
...
<script>
  import {getFinancePayoutInvoiceControl} from '@/Api/invoice/invoiceCollectionApi'
    ...
    data() {
      return {
        ...
        tableColumn: [
          ...
          {field: 'invoiceCreateTime', type: 'html', title: '开票日期', width: 120, align: 'center'},
          {field: 'buyerName', type: 'html', title: '购方', width: 120, align: 'center'},
          {field: 'tax', type: 'html', title: '税额', width: 120, align: 'center'},
          {field: 'invoiceAmountUntax', type: 'html', title: '发票金额(无税)', width: 120, align: 'center'},
          {field: 'residueApprovePaymentAmountTax', type: 'html', title: '剩余可付金额(元)', width: 120, align: 'center'},
          {field: 'sellerName', type: 'html', title: '销售方', width: 120, align: 'center'},
          {field: 'invoiceType', type: 'html', title: '发票类型', width: 120, align: 'center',slots: {
              default: ({ row, column }, h) => {
                return (this.getCustomerStatus(row.invoiceType))
              },
            }},
        ],
    ...
    methods: {
      ...
      loadWindowparms() {
        ...
        if (parmsStr) {
          ...
          // 模块类型
          if (parms.moduleType != undefined) {
            that.searchParams.moduleType = parms.moduleType
          }
      ...
      async getGridData() {
        ...
        const res = await getFinancePayoutInvoiceControl(parm);
        if(res.code === 200){
      ...
      async getDataDictByConfigType() {
        const res = await getDataDictByConfigType({ params: { configTypes: '发票类型' }});
        if (res.success) {
      ...
      getCustomerStatus(status) {
        let text;
        var list = this.invoiceTypeList;
        for(let i in list){
          if(list[i].configValue == status){
            text = list[i].configName
          }
        }
        return text
      },
