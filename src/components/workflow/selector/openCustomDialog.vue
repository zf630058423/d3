  ...
  <div class="f2bpm-page-layout">
    <el-container style="{ height: containerHeight + 'px'}">
      <el-main class="f2bpm-page-right" style="overflow: hidden">
        <!-- <f2bpm-gridsearchform class="grid-search-from" :tableform="tableForm" @expandclick="expandClick" @searchformbtnclick="searchFormbtnClick" @updatevalue="updateFieldValue" /> -->
        <f2bpm-gridsearchform class="grid-search-from" :tableform="tableForm" :child-col="12" :label-width="100" @searchformbtnclick="searchFormbtnClick" />
        <!-- height="full-95" -->
        <!-- <f2bpm-datagrid
          ref="grid"
          grid="grid"
...
export default {
  props: {
    // params参数{code：表名,parm:{isMulti:false,}}
    parms: {
      type: Object,
      default: () => { }
    },
  ...
  watch: {
    ...
    // 'parms.parm.varParams':'varParamsMethods'
  },
  created() {
    ...
    init() {
      ...
      collapsedSider() {
       ...
       loadWindowparms() {
        ...
        const isSetHead = Number(this.parms.isSetHead) || Object.toQueryString('isSetHead') || 0;
        that.windowparms.isSetHead = isSetHead;
        const parms = this.parms.parm;
         if (parms) {
       ...
       loadCustomConfig() {
        ...
        // 格式：[{key:"",value:""}]
        //  that.windowparms.varParams.value = that.windowparms.varParams.value ? Number(that.windowparms.varParams.value) : '';
        var varParams = that.windowparms.varParams != '' ? JSON.stringify(that.windowparms.varParams) : '';
        var parm = { 'isSetHead': that.windowparms.isSetHead, 'code': this.parms.code, 'varParams': varParams };
