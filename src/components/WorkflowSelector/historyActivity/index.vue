    ...
    <div>
      <div class="fence-box " style="width: 220px;">
        ...
        <ul id="list" class="auto-window-height-select">
          <li v-for="item in leftData" class="fence-li-item textClass" :activityid="item.activityId"
              :activityname="item.activityName" :activitycode="item.activityCode"
              @click="leftSelected(item.activityId)">
      ...
      <div class="fence-box right-result-boxlist" style="width: 180px;">
        ...
        <ul id="listResult" class="auto-window-height-select">
          <li v-for="item in rightData" class="fence-li-item textClass" :activityid="item.activityId"
              :activityname="item.activityName" :activitycode="item.activityCode"
              @click="rightSelected(item.activityId)">
            {{ item.stepId }}-{{ item.activityName }}
          </li>
        </ul>
      </div>
