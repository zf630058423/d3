<template>
  <div class="row-layout">
    <form-title v-if="title" :title="title"></form-title>
    <div class="form-layout-row">
       <el-row :gutter="gutter">
        <el-col :span="spanCol(cIndex)" :offset="getOffset(col)" :key="cIndex" v-for="(col, cIndex) in cols">
  ...
  computed: { 
    ...
    spanCol({ cols, offset, cell }) { 
      return (index) => { 
        ...
        if (rCell && !rCell.includes('-') && rCell.length === 1) { //如果只有一位即24
          return 24;
        } else {
          let cLength = rCell.includes('-') ? rCell.split('-').length : 0;
          if (cols === cLength) { //如果格子相同 就按相同的来分配
            return (cols && cols < 8) ? parseInt(((24 - offset * (cols - 1))) / cols) : 1;
          } else { 
            let cellArr = rCell.split('-');
            let total = cellArr.reduce((prev, cur) => { return parseInt(Number(prev) + Number(cur)) }, 0)
